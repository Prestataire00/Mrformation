...
export default function TestConventionPage() {
  ...
  const [dechargeLearners, setDechargeLearners] = useState<LearnerRow[]>([]);
  const [loadingDechargeLearners, setLoadingDechargeLearners] = useState(false);
  const [dechargeBatchSessionId, setDechargeBatchSessionId] = useState<string>("");
  const [generatingDecharge, setGeneratingDecharge] = useState(false);
  const [generatingDechargeBatch, setGeneratingDechargeBatch] = useState(false);
  const [lastDechargeResult, setLastDechargeResult] = useState<{
    engineUsed: string;
    cacheHit: boolean;
    latencyMs: number;
    fileSizeBytes: number;
  } | null>(null);
  const [lastDechargeBatchResult, setLastDechargeBatchResult] = useState<{
    totalLearners: number;
    successCount: number;
    failureCount: number;
    errors: { learnerId: string; learnerName: string; error: string }[];
  ...
  const [habilH0B0InitLearners, setHabilH0B0InitLearners] = useState<LearnerRow[]>([]);
  const [loadingHabilH0B0InitLearners, setLoadingHabilH0B0InitLearners] = useState(false);
  const [habilH0B0InitBatchSessionId, setHabilH0B0InitBatchSessionId] = useState<string>("");
  const [generatingHabilH0B0Init, setGeneratingHabilH0B0Init] = useState(false);
  const [generatingHabilH0B0InitBatch, setGeneratingHabilH0B0InitBatch] = useState(false);
  const [lastHabilH0B0InitResult, setLastHabilH0B0InitResult] = useState<{
    engineUsed: string;
    cacheHit: boolean;
    latencyMs: number;
    fileSizeBytes: number;
  } | null>(null);
  const [lastHabilH0B0InitBatchResult, setLastHabilH0B0InitBatchResult] = useState<{
    totalLearners: number;
    successCount: number;
    failureCount: number;
    errors: { learnerId: string; learnerName: string; error: string }[];
  ...
  // ── Avis Habilitation Électrique H0 B0 (Recyclage Non élec, OPTIONNEL)
  const [habilH0B0SessionId, setHabilH0B0SessionId] = useState<string>("");
  const [habilH0B0LearnerId, setHabilH0B0LearnerId] = useState<string>("");
  const [habilH0B0Learners, setHabilH0B0Learners] = useState<LearnerRow[]>([]);
  const [loadingHabilH0B0Learners, setLoadingHabilH0B0Learners] = useState(false);
  const [habilH0B0BatchSessionId, setHabilH0B0BatchSessionId] = useState<string>("");
  const [generatingHabilH0B0, setGeneratingHabilH0B0] = useState(false);
  const [generatingHabilH0B0Batch, setGeneratingHabilH0B0Batch] = useState(false);
  const [lastHabilH0B0Result, setLastHabilH0B0Result] = useState<{
    engineUsed: string;
    cacheHit: boolean;
    latencyMs: number;
    fileSizeBytes: number;
  } | null>(null);
  const [lastHabilH0B0BatchResult, setLastHabilH0B0BatchResult] = useState<{
    totalLearners: number;
    successCount: number;
    failureCount: number;
    errors: { learnerId: string; learnerName: string; error: string }[];
  ...
  // ── Avis Habilitation Électrique BT-HT (HAUTE TENSION, OPTIONNEL) ───
  const [habilHtSessionId, setHabilHtSessionId] = useState<string>("");
  const [habilHtLearnerId, setHabilHtLearnerId] = useState<string>("");
  const [habilHtLearners, setHabilHtLearners] = useState<LearnerRow[]>([]);
  const [loadingHabilHtLearners, setLoadingHabilHtLearners] = useState(false);
  const [habilHtBatchSessionId, setHabilHtBatchSessionId] = useState<string>("");
  const [generatingHabilHt, setGeneratingHabilHt] = useState(false);
  const [generatingHabilHtBatch, setGeneratingHabilHtBatch] = useState(false);
  const [lastHabilHtResult, setLastHabilHtResult] = useState<{
    engineUsed: string;
    cacheHit: boolean;
    latencyMs: number;
    fileSizeBytes: number;
  } | null>(null);
  const [lastHabilHtBatchResult, setLastHabilHtBatchResult] = useState<{
    totalLearners: number;
    successCount: number;
    failureCount: number;
    errors: { learnerId: string; learnerName: string; error: string }[];
  ...
  // ── Avis Habilitation Électrique H0-B0-BF-HF-BS (sans BE, OPTIONNEL) ─
  const [habilH0SessionId, setHabilH0SessionId] = useState<string>("");
  const [habilH0LearnerId, setHabilH0LearnerId] = useState<string>("");
  const [habilH0Learners, setHabilH0Learners] = useState<LearnerRow[]>([]);
  const [loadingHabilH0Learners, setLoadingHabilH0Learners] = useState(false);
  const [habilH0BatchSessionId, setHabilH0BatchSessionId] = useState<string>("");
  const [generatingHabilH0, setGeneratingHabilH0] = useState(false);
  const [generatingHabilH0Batch, setGeneratingHabilH0Batch] = useState(false);
  const [lastHabilH0Result, setLastHabilH0Result] = useState<{
    engineUsed: string;
    cacheHit: boolean;
    latencyMs: number;
    fileSizeBytes: number;
  } | null>(null);
  const [lastHabilH0BatchResult, setLastHabilH0BatchResult] = useState<{
    totalLearners: number;
    successCount: number;
    failureCount: number;
    errors: { learnerId: string; learnerName: string; error: string }[];
  ...
  // ── Avis Habilitation Électrique B0-BF-BS (norme A2, OPTIONNEL) ─────
  const [habilB0BfBsSessionId, setHabilB0BfBsSessionId] = useState<string>("");
  const [habilB0BfBsLearnerId, setHabilB0BfBsLearnerId] = useState<string>("");
  const [habilB0BfBsLearners, setHabilB0BfBsLearners] = useState<LearnerRow[]>([]);
  const [loadingHabilB0BfBsLearners, setLoadingHabilB0BfBsLearners] = useState(false);
  const [habilB0BfBsBatchSessionId, setHabilB0BfBsBatchSessionId] = useState<string>("");
  const [generatingHabilB0BfBs, setGeneratingHabilB0BfBs] = useState(false);
  const [generatingHabilB0BfBsBatch, setGeneratingHabilB0BfBsBatch] = useState(false);
  const [lastHabilB0BfBsResult, setLastHabilB0BfBsResult] = useState<{
    engineUsed: string;
    cacheHit: boolean;
    latencyMs: number;
    fileSizeBytes: number;
  } | null>(null);
  const [lastHabilB0BfBsBatchResult, setLastHabilB0BfBsBatchResult] = useState<{
    totalLearners: number;
    successCount: number;
    failureCount: number;
    errors: { learnerId: string; learnerName: string; error: string }[];
  ...
  // ── Avis Habilitation Électrique B1V-B2V-BR (sans BC, OPTIONNEL) ────
  const [habilB1vSessionId, setHabilB1vSessionId] = useState<string>("");
  const [habilB1vLearnerId, setHabilB1vLearnerId] = useState<string>("");
  const [habilB1vLearners, setHabilB1vLearners] = useState<LearnerRow[]>([]);
  const [loadingHabilB1vLearners, setLoadingHabilB1vLearners] = useState(false);
  const [habilB1vBatchSessionId, setHabilB1vBatchSessionId] = useState<string>("");
  const [generatingHabilB1v, setGeneratingHabilB1v] = useState(false);
  const [generatingHabilB1vBatch, setGeneratingHabilB1vBatch] = useState(false);
  const [lastHabilB1vResult, setLastHabilB1vResult] = useState<{
    engineUsed: string;
    cacheHit: boolean;
    latencyMs: number;
    fileSizeBytes: number;
  } | null>(null);
  const [lastHabilB1vBatchResult, setLastHabilB1vBatchResult] = useState<{
    totalLearners: number;
    successCount: number;
    failureCount: number;
    errors: { learnerId: string; learnerName: string; error: string }[];
  ...
  // Charge les apprenants pour lettre de décharge single
  useEffect(() => {
    if (!dechargeSessionId) {
      setDechargeLearners([]);
      setDechargeLearnerId("");
      return;
    }
    setLoadingDechargeLearners(true);
    (async () => {
      const { data } = await supabase
        .from("enrollments")
        .select("learner_id, learner:learners(id, first_name, last_name)")
        .eq("session_id", dechargeSessionId);
      setDechargeLearners((data as unknown as LearnerRow[]) || []);
      setLoadingDechargeLearners(false);
    })();
  }, [supabase, dechargeSessionId]);
  ...
  useEffect(() => {
    if (!habilH0B0InitSessionId) {
      setHabilH0B0InitLearners([]);
      setHabilH0B0InitLearnerId("");
      return;
    }
    setLoadingHabilH0B0InitLearners(true);
    (async () => {
      const { data } = await supabase
        .from("enrollments")
        .select("learner_id, learner:learners(id, first_name, last_name)")
        .eq("session_id", habilH0B0InitSessionId);
      setHabilH0B0InitLearners((data as unknown as LearnerRow[]) || []);
      setLoadingHabilH0B0InitLearners(false);
    })();
  }, [supabase, habilH0B0InitSessionId]);

  // Charge les apprenants pour avis habilitation H0 B0 single
  useEffect(() => {
    if (!habilH0B0SessionId) {
      setHabilH0B0Learners([]);
      setHabilH0B0LearnerId("");
      return;
    }
    setLoadingHabilH0B0Learners(true);
    (async () => {
      const { data } = await supabase
        .from("enrollments")
        .select("learner_id, learner:learners(id, first_name, last_name)")
        .eq("session_id", habilH0B0SessionId);
      setHabilH0B0Learners((data as unknown as LearnerRow[]) || []);
      setLoadingHabilH0B0Learners(false);
    })();
  }, [supabase, habilH0B0SessionId]);

  // Charge les apprenants pour avis habilitation BT-HT single
  useEffect(() => {
    if (!habilHtSessionId) {
      setHabilHtLearners([]);
      setHabilHtLearnerId("");
    ...
    setLoadingHabilHtLearners(true);
    (async () => {
      const { data } = await supabase
        .from("enrollments")
        .select("learner_id, learner:learners(id, first_name, last_name)")
        .eq("session_id", habilHtSessionId);
      setHabilHtLearners((data as unknown as LearnerRow[]) || []);
      setLoadingHabilHtLearners(false);
    })();
  }, [supabase, habilHtSessionId]);

  // Charge les apprenants pour avis habilitation H0-B0-BF-HF-BS single
  useEffect(() => {
    if (!habilH0SessionId) {
      setHabilH0Learners([]);
      setHabilH0LearnerId("");
    ...
    setLoadingHabilH0Learners(true);
    (async () => {
      const { data } = await supabase
        .from("enrollments")
        .select("learner_id, learner:learners(id, first_name, last_name)")
        .eq("session_id", habilH0SessionId);
      setHabilH0Learners((data as unknown as LearnerRow[]) || []);
      setLoadingHabilH0Learners(false);
    })();
  }, [supabase, habilH0SessionId]);

  // Charge les apprenants pour avis habilitation B0-BF-BS single
  useEffect(() => {
    if (!habilB0BfBsSessionId) {
      setHabilB0BfBsLearners([]);
      setHabilB0BfBsLearnerId("");
      return;
    }
    setLoadingHabilB0BfBsLearners(true);
    (async () => {
      const { data } = await supabase
        .from("enrollments")
        .select("learner_id, learner:learners(id, first_name, last_name)")
        .eq("session_id", habilB0BfBsSessionId);
      setHabilB0BfBsLearners((data as unknown as LearnerRow[]) || []);
      setLoadingHabilB0BfBsLearners(false);
    })();
  }, [supabase, habilB0BfBsSessionId]);

  // Charge les apprenants pour avis habilitation B1V-B2V-BR single
  useEffect(() => {
    if (!habilB1vSessionId) {
      setHabilB1vLearners([]);
      setHabilB1vLearnerId("");
      return;
    }
    setLoadingHabilB1vLearners(true);
    (async () => {
      const { data } = await supabase
        .from("enrollments")
        .select("learner_id, learner:learners(id, first_name, last_name)")
        .eq("session_id", habilB1vSessionId);
      setHabilB1vLearners((data as unknown as LearnerRow[]) || []);
      setLoadingHabilB1vLearners(false);
    })();
  }, [supabase, habilB1vSessionId]);
  ...
  // ── Lettre de décharge handlers (OPTIONNEL) ─────────────────────────
  async function handleGenerateDechargeMock() {
    setGeneratingDecharge(true);
    setLastDechargeResult(null);
    try {
      const res = await fetch("/api/documents/generate-lettre-decharge-mock", { method: "POST" });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastDechargeResult({
        engineUsed: json.engineUsed, cacheHit: json.cacheHit,
        latencyMs: json.latencyMs, fileSizeBytes: json.fileSizeBytes,
      });
      const bytes = Uint8Array.from(atob(json.pdfBase64), (c) => c.charCodeAt(0));
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Lettre décharge mock générée", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec mock lettre décharge", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingDecharge(false);
    }
  }

  async function handleGenerateDecharge() {
    if (!dechargeSessionId || !dechargeLearnerId) {
      toast({ title: "Sélection incomplète", description: "Choisis session ET apprenant.", variant: "destructive" });
      return;
    }
    setGeneratingDecharge(true);
    setLastDechargeResult(null);
    try {
      const res = await fetch("/api/documents/generate-lettre-decharge", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: dechargeSessionId, learnerId: dechargeLearnerId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastDechargeResult({
      ...
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Lettre décharge générée", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec lettre décharge", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingDecharge(false);
    }
  }

  async function handleGenerateDechargeBatch() {
    if (!dechargeBatchSessionId) {
      toast({ title: "Aucune session", description: "Sélectionne d'abord une session.", variant: "destructive" });
      return;
    }
    setGeneratingDechargeBatch(true);
    setLastDechargeBatchResult(null);
    try {
      const res = await fetch("/api/documents/generate-lettres-decharge-batch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: dechargeBatchSessionId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastDechargeBatchResult({
        totalLearners: json.totalLearners, successCount: json.successCount,
        failureCount: json.failureCount, errors: json.errors ?? [],
        totalLatencyMs: json.totalLatencyMs,
      });
      ...
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      const sessionLabel = sessions.find((s) => s.id === dechargeBatchSessionId)?.title ?? "session";
      a.href = url;
      a.download = `lettres-decharge-${sessionLabel.replace(/[^a-zA-Z0-9-]+/g, "-").toLowerCase().slice(0, 50)}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      toast({
        title: json.failureCount === 0 ? "ZIP lettres décharge généré" : "ZIP avec erreurs",
        description: `${json.successCount}/${json.totalLearners} apprenants · ${json.totalLatencyMs}ms`,
        variant: json.failureCount === 0 ? "default" : "destructive",
      });
    } catch (err) {
      toast({ title: "Échec batch lettres décharge", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingDechargeBatch(false);
    }
  }
  ...
  // ── Avis Habilitation Électrique H0 B0 INITIAL handlers (OPTIONNEL) ─
  async function handleGenerateHabilH0B0InitMock() {
    setGeneratingHabilH0B0Init(true);
    setLastHabilH0B0InitResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-h0-b0-initial-mock", { method: "POST" });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilH0B0InitResult({
        engineUsed: json.engineUsed, cacheHit: json.cacheHit,
        latencyMs: json.latencyMs, fileSizeBytes: json.fileSizeBytes,
      });
      const bytes = Uint8Array.from(atob(json.pdfBase64), (c) => c.charCodeAt(0));
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Avis habilitation H0 B0 Initial mock généré", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec mock avis habilitation H0 B0 Initial", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilH0B0Init(false);
    }
  }

  async function handleGenerateHabilH0B0Init() {
    if (!habilH0B0InitSessionId || !habilH0B0InitLearnerId) {
      toast({ title: "Sélection incomplète", description: "Choisis session ET apprenant.", variant: "destructive" });
      return;
    }
    setGeneratingHabilH0B0Init(true);
    setLastHabilH0B0InitResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-h0-b0-initial", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: habilH0B0InitSessionId, learnerId: habilH0B0InitLearnerId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilH0B0InitResult({
      ...
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Avis habilitation H0 B0 Initial généré", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec avis habilitation H0 B0 Initial", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilH0B0Init(false);
    }
  }

  async function handleGenerateHabilH0B0InitBatch() {
    if (!habilH0B0InitBatchSessionId) {
      toast({ title: "Aucune session", description: "Sélectionne d'abord une session.", variant: "destructive" });
      return;
    }
    setGeneratingHabilH0B0InitBatch(true);
    setLastHabilH0B0InitBatchResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-h0-b0-initial-batch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: habilH0B0InitBatchSessionId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilH0B0InitBatchResult({
        totalLearners: json.totalLearners, successCount: json.successCount,
        failureCount: json.failureCount, errors: json.errors ?? [],
        totalLatencyMs: json.totalLatencyMs,
      });
      ...
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      const sessionLabel = sessions.find((s) => s.id === habilH0B0InitBatchSessionId)?.title ?? "session";
      a.href = url;
      a.download = `avis-habilitation-h0-b0-initial-${sessionLabel.replace(/[^a-zA-Z0-9-]+/g, "-").toLowerCase().slice(0, 50)}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      toast({
        title: json.failureCount === 0 ? "ZIP avis habilitation H0 B0 Initial généré" : "ZIP avec erreurs",
        description: `${json.successCount}/${json.totalLearners} apprenants · ${json.totalLatencyMs}ms`,
        variant: json.failureCount === 0 ? "default" : "destructive",
      });
    } catch (err) {
      toast({ title: "Échec batch avis habilitation H0 B0 Initial", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilH0B0InitBatch(false);
    }
  }

  // ── Avis Habilitation Électrique H0 B0 handlers (OPTIONNEL) ─────────
  async function handleGenerateHabilH0B0Mock() {
    setGeneratingHabilH0B0(true);
    setLastHabilH0B0Result(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-h0-b0-mock", { method: "POST" });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilH0B0Result({
        engineUsed: json.engineUsed, cacheHit: json.cacheHit,
        latencyMs: json.latencyMs, fileSizeBytes: json.fileSizeBytes,
      });
      const bytes = Uint8Array.from(atob(json.pdfBase64), (c) => c.charCodeAt(0));
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Avis habilitation H0 B0 mock généré", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec mock avis habilitation H0 B0", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilH0B0(false);
    }
  }

  async function handleGenerateHabilH0B0() {
    if (!habilH0B0SessionId || !habilH0B0LearnerId) {
      toast({ title: "Sélection incomplète", description: "Choisis session ET apprenant.", variant: "destructive" });
      return;
    }
    setGeneratingHabilH0B0(true);
    setLastHabilH0B0Result(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-h0-b0", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: habilH0B0SessionId, learnerId: habilH0B0LearnerId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilH0B0Result({
      ...
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Avis habilitation H0 B0 généré", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec avis habilitation H0 B0", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilH0B0(false);
    }
  }

  async function handleGenerateHabilH0B0Batch() {
    if (!habilH0B0BatchSessionId) {
      toast({ title: "Aucune session", description: "Sélectionne d'abord une session.", variant: "destructive" });
      return;
    }
    setGeneratingHabilH0B0Batch(true);
    setLastHabilH0B0BatchResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-h0-b0-batch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: habilH0B0BatchSessionId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilH0B0BatchResult({
        totalLearners: json.totalLearners, successCount: json.successCount,
        failureCount: json.failureCount, errors: json.errors ?? [],
        totalLatencyMs: json.totalLatencyMs,
      });
      ...
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      const sessionLabel = sessions.find((s) => s.id === habilH0B0BatchSessionId)?.title ?? "session";
      a.href = url;
      a.download = `avis-habilitation-h0-b0-${sessionLabel.replace(/[^a-zA-Z0-9-]+/g, "-").toLowerCase().slice(0, 50)}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      toast({
        title: json.failureCount === 0 ? "ZIP avis habilitation H0 B0 généré" : "ZIP avec erreurs",
        description: `${json.successCount}/${json.totalLearners} apprenants · ${json.totalLatencyMs}ms`,
        variant: json.failureCount === 0 ? "default" : "destructive",
      });
    } catch (err) {
      toast({ title: "Échec batch avis habilitation H0 B0", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilH0B0Batch(false);
    }
  }

  // ── Avis Habilitation Électrique BT-HT handlers (OPTIONNEL) ─────────
  async function handleGenerateHabilHtMock() {
    setGeneratingHabilHt(true);
    setLastHabilHtResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-bt-ht-mock", { method: "POST" });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilHtResult({
        engineUsed: json.engineUsed, cacheHit: json.cacheHit,
        latencyMs: json.latencyMs, fileSizeBytes: json.fileSizeBytes,
      });
      const bytes = Uint8Array.from(atob(json.pdfBase64), (c) => c.charCodeAt(0));
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Avis habilitation BT-HT mock généré", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec mock avis habilitation BT-HT", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilHt(false);
    }
  }

  async function handleGenerateHabilHt() {
    if (!habilHtSessionId || !habilHtLearnerId) {
      toast({ title: "Sélection incomplète", description: "Choisis session ET apprenant.", variant: "destructive" });
      return;
    }
    setGeneratingHabilHt(true);
    setLastHabilHtResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-bt-ht", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: habilHtSessionId, learnerId: habilHtLearnerId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilHtResult({
      ...
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Avis habilitation BT-HT généré", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec avis habilitation BT-HT", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilHt(false);
    }
  }

  async function handleGenerateHabilHtBatch() {
    if (!habilHtBatchSessionId) {
      toast({ title: "Aucune session", description: "Sélectionne d'abord une session.", variant: "destructive" });
      return;
    }
    setGeneratingHabilHtBatch(true);
    setLastHabilHtBatchResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-bt-ht-batch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: habilHtBatchSessionId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilHtBatchResult({
        totalLearners: json.totalLearners, successCount: json.successCount,
        failureCount: json.failureCount, errors: json.errors ?? [],
        totalLatencyMs: json.totalLatencyMs,
      });
      ...
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      const sessionLabel = sessions.find((s) => s.id === habilHtBatchSessionId)?.title ?? "session";
      a.href = url;
      a.download = `avis-habilitation-bt-ht-${sessionLabel.replace(/[^a-zA-Z0-9-]+/g, "-").toLowerCase().slice(0, 50)}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      toast({
        title: json.failureCount === 0 ? "ZIP avis habilitation BT-HT généré" : "ZIP avec erreurs",
        description: `${json.successCount}/${json.totalLearners} apprenants · ${json.totalLatencyMs}ms`,
        variant: json.failureCount === 0 ? "default" : "destructive",
      });
    } catch (err) {
      toast({ title: "Échec batch avis habilitation BT-HT", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilHtBatch(false);
    }
  }

  // ── Avis Habilitation Électrique H0-B0-BF-HF-BS handlers (OPTIONNEL) ─
  async function handleGenerateHabilH0Mock() {
    setGeneratingHabilH0(true);
    setLastHabilH0Result(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-h0-b0-bf-hf-bs-mock", { method: "POST" });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilH0Result({
        engineUsed: json.engineUsed, cacheHit: json.cacheHit,
        latencyMs: json.latencyMs, fileSizeBytes: json.fileSizeBytes,
      });
      const bytes = Uint8Array.from(atob(json.pdfBase64), (c) => c.charCodeAt(0));
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Avis habilitation H0-B0-BF-HF-BS mock généré", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec mock avis habilitation H0-B0-BF-HF-BS", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilH0(false);
    }
  }

  async function handleGenerateHabilH0() {
    if (!habilH0SessionId || !habilH0LearnerId) {
      toast({ title: "Sélection incomplète", description: "Choisis session ET apprenant.", variant: "destructive" });
      return;
    }
    setGeneratingHabilH0(true);
    setLastHabilH0Result(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-h0-b0-bf-hf-bs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: habilH0SessionId, learnerId: habilH0LearnerId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilH0Result({
      ...
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Avis habilitation H0-B0-BF-HF-BS généré", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec avis habilitation H0-B0-BF-HF-BS", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilH0(false);
    }
  }

  async function handleGenerateHabilH0Batch() {
    if (!habilH0BatchSessionId) {
      toast({ title: "Aucune session", description: "Sélectionne d'abord une session.", variant: "destructive" });
      return;
    }
    setGeneratingHabilH0Batch(true);
    setLastHabilH0BatchResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-h0-b0-bf-hf-bs-batch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: habilH0BatchSessionId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilH0BatchResult({
        totalLearners: json.totalLearners, successCount: json.successCount,
        failureCount: json.failureCount, errors: json.errors ?? [],
        totalLatencyMs: json.totalLatencyMs,
      });
      ...
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      const sessionLabel = sessions.find((s) => s.id === habilH0BatchSessionId)?.title ?? "session";
      a.href = url;
      a.download = `avis-habilitation-h0-b0-bf-hf-bs-${sessionLabel.replace(/[^a-zA-Z0-9-]+/g, "-").toLowerCase().slice(0, 50)}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      toast({
        title: json.failureCount === 0 ? "ZIP avis habilitation H0-B0-BF-HF-BS généré" : "ZIP avec erreurs",
        description: `${json.successCount}/${json.totalLearners} apprenants · ${json.totalLatencyMs}ms`,
        variant: json.failureCount === 0 ? "default" : "destructive",
      });
    } catch (err) {
      toast({ title: "Échec batch avis habilitation H0-B0-BF-HF-BS", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilH0Batch(false);
    }
  }

  // ── Avis Habilitation Électrique B0-BF-BS handlers (OPTIONNEL) ──────
  async function handleGenerateHabilB0BfBsMock() {
    setGeneratingHabilB0BfBs(true);
    setLastHabilB0BfBsResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-b0-bf-bs-mock", { method: "POST" });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilB0BfBsResult({
        engineUsed: json.engineUsed, cacheHit: json.cacheHit,
        latencyMs: json.latencyMs, fileSizeBytes: json.fileSizeBytes,
      });
      const bytes = Uint8Array.from(atob(json.pdfBase64), (c) => c.charCodeAt(0));
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Avis habilitation B0-BF-BS mock généré", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec mock avis habilitation B0-BF-BS", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilB0BfBs(false);
    }
  }

  async function handleGenerateHabilB0BfBs() {
    if (!habilB0BfBsSessionId || !habilB0BfBsLearnerId) {
      toast({ title: "Sélection incomplète", description: "Choisis session ET apprenant.", variant: "destructive" });
      return;
    }
    setGeneratingHabilB0BfBs(true);
    setLastHabilB0BfBsResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-b0-bf-bs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: habilB0BfBsSessionId, learnerId: habilB0BfBsLearnerId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilB0BfBsResult({
      ...
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Avis habilitation B0-BF-BS généré", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec avis habilitation B0-BF-BS", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilB0BfBs(false);
    }
  }

  async function handleGenerateHabilB0BfBsBatch() {
    if (!habilB0BfBsBatchSessionId) {
      toast({ title: "Aucune session", description: "Sélectionne d'abord une session.", variant: "destructive" });
      return;
    }
    setGeneratingHabilB0BfBsBatch(true);
    setLastHabilB0BfBsBatchResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-b0-bf-bs-batch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: habilB0BfBsBatchSessionId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilB0BfBsBatchResult({
        totalLearners: json.totalLearners, successCount: json.successCount,
        failureCount: json.failureCount, errors: json.errors ?? [],
        totalLatencyMs: json.totalLatencyMs,
      });
      ...
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      const sessionLabel = sessions.find((s) => s.id === habilB0BfBsBatchSessionId)?.title ?? "session";
      a.href = url;
      a.download = `avis-habilitation-b0-bf-bs-${sessionLabel.replace(/[^a-zA-Z0-9-]+/g, "-").toLowerCase().slice(0, 50)}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      toast({
        title: json.failureCount === 0 ? "ZIP avis habilitation B0-BF-BS généré" : "ZIP avec erreurs",
        description: `${json.successCount}/${json.totalLearners} apprenants · ${json.totalLatencyMs}ms`,
        variant: json.failureCount === 0 ? "default" : "destructive",
      });
    } catch (err) {
      toast({ title: "Échec batch avis habilitation B0-BF-BS", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilB0BfBsBatch(false);
    }
  }

  // ── Avis Habilitation Électrique B1V-B2V-BR handlers (OPTIONNEL) ────
  async function handleGenerateHabilB1vMock() {
    setGeneratingHabilB1v(true);
    setLastHabilB1vResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-b1v-b2v-br-mock", { method: "POST" });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilB1vResult({
        engineUsed: json.engineUsed, cacheHit: json.cacheHit,
        latencyMs: json.latencyMs, fileSizeBytes: json.fileSizeBytes,
      });
      const bytes = Uint8Array.from(atob(json.pdfBase64), (c) => c.charCodeAt(0));
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Avis habilitation B1V-B2V-BR mock généré", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec mock avis habilitation B1V-B2V-BR", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilB1v(false);
    }
  }

  async function handleGenerateHabilB1v() {
    if (!habilB1vSessionId || !habilB1vLearnerId) {
      toast({ title: "Sélection incomplète", description: "Choisis session ET apprenant.", variant: "destructive" });
      return;
    }
    setGeneratingHabilB1v(true);
    setLastHabilB1vResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-b1v-b2v-br", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: habilB1vSessionId, learnerId: habilB1vLearnerId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilB1vResult({
      ...
      const blob = new Blob([bytes], { type: "application/pdf" });
      window.open(URL.createObjectURL(blob), "_blank");
      toast({ title: "Avis habilitation B1V-B2V-BR généré", description: `${json.engineUsed} · ${json.latencyMs}ms` });
    } catch (err) {
      toast({ title: "Échec avis habilitation B1V-B2V-BR", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilB1v(false);
    }
  }

  async function handleGenerateHabilB1vBatch() {
    if (!habilB1vBatchSessionId) {
      toast({ title: "Aucune session", description: "Sélectionne d'abord une session.", variant: "destructive" });
      return;
    }
    setGeneratingHabilB1vBatch(true);
    setLastHabilB1vBatchResult(null);
    try {
      const res = await fetch("/api/documents/generate-avis-habilitation-electrique-b1v-b2v-br-batch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: habilB1vBatchSessionId }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setLastHabilB1vBatchResult({
        totalLearners: json.totalLearners, successCount: json.successCount,
        failureCount: json.failureCount, errors: json.errors ?? [],
        totalLatencyMs: json.totalLatencyMs,
      });
      ...
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      const sessionLabel = sessions.find((s) => s.id === habilB1vBatchSessionId)?.title ?? "session";
      a.href = url;
      a.download = `avis-habilitation-b1v-b2v-br-${sessionLabel.replace(/[^a-zA-Z0-9-]+/g, "-").toLowerCase().slice(0, 50)}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      toast({
        title: json.failureCount === 0 ? "ZIP avis habilitation B1V-B2V-BR généré" : "ZIP avec erreurs",
        description: `${json.successCount}/${json.totalLearners} apprenants · ${json.totalLatencyMs}ms`,
        variant: json.failureCount === 0 ? "default" : "destructive",
      });
    } catch (err) {
      toast({ title: "Échec batch avis habilitation B1V-B2V-BR", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
    } finally {
      setGeneratingHabilB1vBatch(false);
    }
  }
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4 space-y-6">
      ...
      {/* ────────── Avis Habilitation Électrique B1V-B2V-BR (sans BC) ────────── */}
      ...
      <div className="pt-8">
        ...
        <p className="text-sm text-muted-foreground">
          4ème variante — identique à la V1 avancée (norme A1) mais
          <strong> sans BC</strong> (Chargé de consignation vide). Symboles :
          BO-H0V, B1 B1V, B2 B2V, BR, BE ESSAI.
        </p>
      </div>
      ...
      <Card className="border-emerald-300 bg-emerald-50/40">
        ...
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Patrick ATTLAN + HAB ELEC B1V B2V BR - INITIAL (sans BC).
          </p>
          <Button
            onClick={handleGenerateHabilB1vMock}
            disabled={generatingHabilB1v || generatingHabilB1vBatch}
            className="w-full gap-2 bg-emerald-700 hover:bg-emerald-800"
          >
            {generatingHabilB1v ? <Loader2 className="h-4 w-4 animate-spin" /> : <FlaskConical className="h-4 w-4" />}
            Générer avis habilitation B1V-B2V-BR de test
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Avis habilitation B1V-B2V-BR — Données réelles (1 apprenant)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="habil-b1v-session-select" className="text-sm">Session</Label>
            <Select value={habilB1vSessionId} onValueChange={setHabilB1vSessionId}
              disabled={loadingSessions || generatingHabilB1v}>
              <SelectTrigger id="habil-b1v-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <div>
            <Label htmlFor="habil-b1v-learner-select" className="text-sm">Apprenant inscrit</Label>
            <Select value={habilB1vLearnerId} onValueChange={setHabilB1vLearnerId}
              disabled={!habilB1vSessionId || loadingHabilB1vLearners || generatingHabilB1v}>
              <SelectTrigger id="habil-b1v-learner-select" className="mt-1">
                <SelectValue placeholder={
                  !habilB1vSessionId ? "Choisis d'abord une session"
                    : loadingHabilB1vLearners ? "Chargement…"
                      : habilB1vLearners.length === 0 ? "Aucun apprenant inscrit"
                        : "Choisir un apprenant…"
                } />
              </SelectTrigger>
              <SelectContent>
                {habilB1vLearners.map((l) => l.learner ? (
                  <SelectItem key={l.learner_id} value={l.learner_id}>
                    {l.learner.last_name} {l.learner.first_name}
                  </SelectItem>
                ) : null)}
          ...
          <Button onClick={handleGenerateHabilB1v}
            disabled={!habilB1vSessionId || !habilB1vLearnerId || generatingHabilB1v}
            className="w-full gap-2" size="lg">
            {generatingHabilB1v ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><Zap className="h-4 w-4" />Générer avis habilitation B1V-B2V-BR</>}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card className="border-purple-200 bg-purple-50/30">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Génère <strong>1 avis B1V-B2V-BR par apprenant</strong>. Sortie : 1 ZIP.
          </p>
          <div>
            <Label htmlFor="habil-b1v-batch-session-select" className="text-sm">Session</Label>
            <Select value={habilB1vBatchSessionId} onValueChange={setHabilB1vBatchSessionId}
              disabled={loadingSessions || generatingHabilB1vBatch}>
              <SelectTrigger id="habil-b1v-batch-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <Button onClick={handleGenerateHabilB1vBatch}
            disabled={!habilB1vBatchSessionId || generatingHabilB1vBatch}
            className="w-full gap-2 bg-purple-600 hover:bg-purple-700" size="lg">
            {generatingHabilB1vBatch ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><Package className="h-4 w-4" />Générer ZIP — tous les avis B1V-B2V-BR</>}
          </Button>
        </CardContent>
      </Card>

      {lastHabilB1vBatchResult && (
        <Card className={lastHabilB1vBatchResult.failureCount === 0
          ? "border-green-200 bg-green-50/30" : "border-amber-200 bg-amber-50/30"}>
          <CardHeader>
            <CardTitle className={"text-base " + (lastHabilB1vBatchResult.failureCount === 0 ? "text-green-900" : "text-amber-900")}>
              {lastHabilB1vBatchResult.failureCount === 0 ? "✅" : "⚠️"} Dernier batch avis habilitation B1V-B2V-BR
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div><strong>{lastHabilB1vBatchResult.successCount}</strong> / {lastHabilB1vBatchResult.totalLearners} avis générés</div>
            <div><strong>Latence totale :</strong> {lastHabilB1vBatchResult.totalLatencyMs} ms</div>
            {lastHabilB1vBatchResult.errors.length > 0 && (
              <div className="mt-3 space-y-1">
                <div className="flex items-center gap-1 text-amber-900 font-medium">
                  <AlertCircle className="h-4 w-4" /> Erreurs ({lastHabilB1vBatchResult.errors.length})
                </div>
                <ul className="text-xs space-y-0.5 ml-5 list-disc">
                  {lastHabilB1vBatchResult.errors.map((e) => (
                    <li key={e.learnerId}><strong>{e.learnerName}</strong> : {e.error}</li>
                  ))}
                </ul>
              </div>
      ...
      {lastHabilB1vResult && (
        <Card className="border-green-200 bg-green-50/30">
          <CardHeader><CardTitle className="text-base text-green-900">✅ Dernier avis habilitation B1V-B2V-BR</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div>
              <strong>Moteur :</strong> {lastHabilB1vResult.engineUsed}
              {lastHabilB1vResult.cacheHit && (
                <span className="ml-2 text-xs bg-amber-100 text-amber-800 px-2 py-0.5 rounded">⚡ Cache hit</span>
              )}
            </div>
            <div><strong>Latence :</strong> {lastHabilB1vResult.latencyMs} ms</div>
            <div><strong>Taille PDF :</strong> {(lastHabilB1vResult.fileSizeBytes / 1024).toFixed(1)} KB</div>
          </CardContent>
        </Card>
      )}

      {/* ────────── Avis Habilitation Électrique B0-BF-BS (norme A2) ────────── */}

      <div className="pt-8">
        <h2 className="text-xl font-semibold flex items-center gap-2 mb-1">
          <Zap className="h-5 w-5 text-lime-700" />
          Avis Habilitation Électrique — B0 H0 BF HF BS BE Manoeuvre
        </h2>
        <p className="text-sm text-muted-foreground">
          5ème variante — norme A2. Symboles : Exécutant Non élec
          <strong> BO-H0 / BF-HF</strong> (stackés), Chargé de chantier
          <strong> BO-H0</strong>, Exécutant élec <strong>BS</strong>,
          Chargé d&apos;opérations spécifiques <strong>BE MANOEUVRE</strong>.
        </p>
      </div>
      ...
      <Card className="border-lime-300 bg-lime-50/40">
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <FlaskConical className="h-4 w-4 text-lime-800" />
            Mode rapide — Données factices
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Patrick ATTLAN + Habilitation électrique INITIAL B0 H0 BF HF BS BE MANŒUVRE.
          </p>
          <Button
            onClick={handleGenerateHabilB0BfBsMock}
            disabled={generatingHabilB0BfBs || generatingHabilB0BfBsBatch}
            className="w-full gap-2 bg-lime-700 hover:bg-lime-800"
          >
            {generatingHabilB0BfBs ? <Loader2 className="h-4 w-4 animate-spin" /> : <FlaskConical className="h-4 w-4" />}
            Générer avis habilitation B0-BF-BS de test
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Avis habilitation B0-BF-BS — Données réelles (1 apprenant)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="habil-b0bfbs-session-select" className="text-sm">Session</Label>
            <Select value={habilB0BfBsSessionId} onValueChange={setHabilB0BfBsSessionId}
              disabled={loadingSessions || generatingHabilB0BfBs}>
              <SelectTrigger id="habil-b0bfbs-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <div>
            <Label htmlFor="habil-b0bfbs-learner-select" className="text-sm">Apprenant inscrit</Label>
            <Select value={habilB0BfBsLearnerId} onValueChange={setHabilB0BfBsLearnerId}
              disabled={!habilB0BfBsSessionId || loadingHabilB0BfBsLearners || generatingHabilB0BfBs}>
              <SelectTrigger id="habil-b0bfbs-learner-select" className="mt-1">
                <SelectValue placeholder={
                  !habilB0BfBsSessionId ? "Choisis d'abord une session"
                    : loadingHabilB0BfBsLearners ? "Chargement…"
                      : habilB0BfBsLearners.length === 0 ? "Aucun apprenant inscrit"
                        : "Choisir un apprenant…"
                } />
              </SelectTrigger>
              <SelectContent>
                {habilB0BfBsLearners.map((l) => l.learner ? (
                  <SelectItem key={l.learner_id} value={l.learner_id}>
                    {l.learner.last_name} {l.learner.first_name}
                  </SelectItem>
                ) : null)}
          ...
          <Button onClick={handleGenerateHabilB0BfBs}
            disabled={!habilB0BfBsSessionId || !habilB0BfBsLearnerId || generatingHabilB0BfBs}
            className="w-full gap-2" size="lg">
            {generatingHabilB0BfBs ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><Zap className="h-4 w-4" />Générer avis habilitation B0-BF-BS</>}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card className="border-purple-200 bg-purple-50/30">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Génère <strong>1 avis B0-BF-BS par apprenant</strong>. Sortie : 1 ZIP.
          </p>
          <div>
            <Label htmlFor="habil-b0bfbs-batch-session-select" className="text-sm">Session</Label>
            <Select value={habilB0BfBsBatchSessionId} onValueChange={setHabilB0BfBsBatchSessionId}
              disabled={loadingSessions || generatingHabilB0BfBsBatch}>
              <SelectTrigger id="habil-b0bfbs-batch-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <Button onClick={handleGenerateHabilB0BfBsBatch}
            disabled={!habilB0BfBsBatchSessionId || generatingHabilB0BfBsBatch}
            className="w-full gap-2 bg-purple-600 hover:bg-purple-700" size="lg">
            {generatingHabilB0BfBsBatch ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><Package className="h-4 w-4" />Générer ZIP — tous les avis B0-BF-BS</>}
          </Button>
        </CardContent>
      </Card>

      {lastHabilB0BfBsBatchResult && (
        <Card className={lastHabilB0BfBsBatchResult.failureCount === 0
          ? "border-green-200 bg-green-50/30" : "border-amber-200 bg-amber-50/30"}>
          <CardHeader>
            <CardTitle className={"text-base " + (lastHabilB0BfBsBatchResult.failureCount === 0 ? "text-green-900" : "text-amber-900")}>
              {lastHabilB0BfBsBatchResult.failureCount === 0 ? "✅" : "⚠️"} Dernier batch avis habilitation B0-BF-BS
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div><strong>{lastHabilB0BfBsBatchResult.successCount}</strong> / {lastHabilB0BfBsBatchResult.totalLearners} avis générés</div>
            <div><strong>Latence totale :</strong> {lastHabilB0BfBsBatchResult.totalLatencyMs} ms</div>
            {lastHabilB0BfBsBatchResult.errors.length > 0 && (
              <div className="mt-3 space-y-1">
                <div className="flex items-center gap-1 text-amber-900 font-medium">
                  <AlertCircle className="h-4 w-4" /> Erreurs ({lastHabilB0BfBsBatchResult.errors.length})
                </div>
                <ul className="text-xs space-y-0.5 ml-5 list-disc">
                  {lastHabilB0BfBsBatchResult.errors.map((e) => (
                    <li key={e.learnerId}><strong>{e.learnerName}</strong> : {e.error}</li>
                  ))}
                </ul>
              </div>
      ...
      {lastHabilB0BfBsResult && (
        <Card className="border-green-200 bg-green-50/30">
          <CardHeader><CardTitle className="text-base text-green-900">✅ Dernier avis habilitation B0-BF-BS</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div>
              <strong>Moteur :</strong> {lastHabilB0BfBsResult.engineUsed}
              {lastHabilB0BfBsResult.cacheHit && (
                <span className="ml-2 text-xs bg-amber-100 text-amber-800 px-2 py-0.5 rounded">⚡ Cache hit</span>
              )}
            </div>
            <div><strong>Latence :</strong> {lastHabilB0BfBsResult.latencyMs} ms</div>
            <div><strong>Taille PDF :</strong> {(lastHabilB0BfBsResult.fileSizeBytes / 1024).toFixed(1)} KB</div>
          </CardContent>
        </Card>
      )}

      {/* ────────── Avis Habilitation Électrique H0-B0-BF-HF-BS (sans BE) ────────── */}
      ...
      <div className="pt-8">
        ...
        <p className="text-sm text-muted-foreground">
          6ème variante — identique à B0-BF-BS mais{" "}
          <strong>sans BE MANOEUVRE</strong> (Chargé d&apos;opérations
          spécifiques vide). Norme A2.
        </p>
      </div>
      ...
      <Card className="border-cyan-300 bg-cyan-50/40">
        ...
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Patrick ATTLAN + Habilitation électrique Initial H0 B0 BF HF BS.
          </p>
          <Button
            onClick={handleGenerateHabilH0Mock}
            disabled={generatingHabilH0 || generatingHabilH0Batch}
            className="w-full gap-2 bg-cyan-700 hover:bg-cyan-800"
          >
            {generatingHabilH0 ? <Loader2 className="h-4 w-4 animate-spin" /> : <FlaskConical className="h-4 w-4" />}
            Générer avis habilitation H0-B0-BF-HF-BS de test
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Avis habilitation H0-B0-BF-HF-BS — Données réelles (1 apprenant)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="habil-h0-session-select" className="text-sm">Session</Label>
            <Select value={habilH0SessionId} onValueChange={setHabilH0SessionId}
              disabled={loadingSessions || generatingHabilH0}>
              <SelectTrigger id="habil-h0-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <div>
            <Label htmlFor="habil-h0-learner-select" className="text-sm">Apprenant inscrit</Label>
            <Select value={habilH0LearnerId} onValueChange={setHabilH0LearnerId}
              disabled={!habilH0SessionId || loadingHabilH0Learners || generatingHabilH0}>
              <SelectTrigger id="habil-h0-learner-select" className="mt-1">
                <SelectValue placeholder={
                  !habilH0SessionId ? "Choisis d'abord une session"
                    : loadingHabilH0Learners ? "Chargement…"
                      : habilH0Learners.length === 0 ? "Aucun apprenant inscrit"
                        : "Choisir un apprenant…"
                } />
              </SelectTrigger>
              <SelectContent>
                {habilH0Learners.map((l) => l.learner ? (
                  <SelectItem key={l.learner_id} value={l.learner_id}>
                    {l.learner.last_name} {l.learner.first_name}
                  </SelectItem>
                ) : null)}
          ...
          <Button onClick={handleGenerateHabilH0}
            disabled={!habilH0SessionId || !habilH0LearnerId || generatingHabilH0}
            className="w-full gap-2" size="lg">
            {generatingHabilH0 ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><Zap className="h-4 w-4" />Générer avis habilitation H0-B0-BF-HF-BS</>}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card className="border-purple-200 bg-purple-50/30">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Génère <strong>1 avis H0-B0-BF-HF-BS par apprenant</strong>. Sortie : 1 ZIP.
          </p>
          <div>
            <Label htmlFor="habil-h0-batch-session-select" className="text-sm">Session</Label>
            <Select value={habilH0BatchSessionId} onValueChange={setHabilH0BatchSessionId}
              disabled={loadingSessions || generatingHabilH0Batch}>
              <SelectTrigger id="habil-h0-batch-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <Button onClick={handleGenerateHabilH0Batch}
            disabled={!habilH0BatchSessionId || generatingHabilH0Batch}
            className="w-full gap-2 bg-purple-600 hover:bg-purple-700" size="lg">
            {generatingHabilH0Batch ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><Package className="h-4 w-4" />Générer ZIP — tous les avis H0-B0-BF-HF-BS</>}
          </Button>
        </CardContent>
      </Card>

      {lastHabilH0BatchResult && (
        <Card className={lastHabilH0BatchResult.failureCount === 0
          ? "border-green-200 bg-green-50/30" : "border-amber-200 bg-amber-50/30"}>
          <CardHeader>
            <CardTitle className={"text-base " + (lastHabilH0BatchResult.failureCount === 0 ? "text-green-900" : "text-amber-900")}>
              {lastHabilH0BatchResult.failureCount === 0 ? "✅" : "⚠️"} Dernier batch avis habilitation H0-B0-BF-HF-BS
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div><strong>{lastHabilH0BatchResult.successCount}</strong> / {lastHabilH0BatchResult.totalLearners} avis générés</div>
            <div><strong>Latence totale :</strong> {lastHabilH0BatchResult.totalLatencyMs} ms</div>
            {lastHabilH0BatchResult.errors.length > 0 && (
              <div className="mt-3 space-y-1">
                <div className="flex items-center gap-1 text-amber-900 font-medium">
                  <AlertCircle className="h-4 w-4" /> Erreurs ({lastHabilH0BatchResult.errors.length})
                </div>
                <ul className="text-xs space-y-0.5 ml-5 list-disc">
                  {lastHabilH0BatchResult.errors.map((e) => (
                    <li key={e.learnerId}><strong>{e.learnerName}</strong> : {e.error}</li>
                  ))}
                </ul>
              </div>
      ...
      {lastHabilH0Result && (
        <Card className="border-green-200 bg-green-50/30">
          <CardHeader><CardTitle className="text-base text-green-900">✅ Dernier avis habilitation H0-B0-BF-HF-BS</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div>
              <strong>Moteur :</strong> {lastHabilH0Result.engineUsed}
              {lastHabilH0Result.cacheHit && (
                <span className="ml-2 text-xs bg-amber-100 text-amber-800 px-2 py-0.5 rounded">⚡ Cache hit</span>
              )}
            </div>
            <div><strong>Latence :</strong> {lastHabilH0Result.latencyMs} ms</div>
            <div><strong>Taille PDF :</strong> {(lastHabilH0Result.fileSizeBytes / 1024).toFixed(1)} KB</div>
          </CardContent>
        </Card>
      )}

      {/* ────────── Avis Habilitation Électrique BT-HT (HAUTE TENSION) ────────── */}
      ...
      <div className="pt-8">
        ...
        <p className="text-sm text-muted-foreground">
          7ème variante — la plus complète. Norme A2. Première à utiliser la
          colonne <strong>Domaine de tension</strong> (BT / HTA). Toutes les
          lignes Personnel remplies sauf &ldquo;Habilité spécial&rdquo;.
          Inclut : BO-H0V, B1 B1V/H1 H1V, B2 B2V/H2 H2V, BR HR, BC HC,
          BS BE MANOEUVRE/BE ESSAI (BT), HE VERIFICATION/HE ESSAI (HTA).
        </p>
      </div>
      ...
      <Card className="border-red-300 bg-red-50/40">
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <FlaskConical className="h-4 w-4 text-red-800" />
            Mode rapide — Données factices
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Patrick ATTLAN + HABILITATION HAUTE TENSION (tableau complet BT + HTA).
          </p>
          <Button
            onClick={handleGenerateHabilHtMock}
            disabled={generatingHabilHt || generatingHabilHtBatch}
            className="w-full gap-2 bg-red-700 hover:bg-red-800"
          >
            {generatingHabilHt ? <Loader2 className="h-4 w-4 animate-spin" /> : <FlaskConical className="h-4 w-4" />}
            Générer avis habilitation BT-HT de test
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Avis habilitation BT-HT — Données réelles (1 apprenant)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="habil-ht-session-select" className="text-sm">Session</Label>
            <Select value={habilHtSessionId} onValueChange={setHabilHtSessionId}
              disabled={loadingSessions || generatingHabilHt}>
              <SelectTrigger id="habil-ht-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <div>
            <Label htmlFor="habil-ht-learner-select" className="text-sm">Apprenant inscrit</Label>
            <Select value={habilHtLearnerId} onValueChange={setHabilHtLearnerId}
              disabled={!habilHtSessionId || loadingHabilHtLearners || generatingHabilHt}>
              <SelectTrigger id="habil-ht-learner-select" className="mt-1">
                <SelectValue placeholder={
                  !habilHtSessionId ? "Choisis d'abord une session"
                    : loadingHabilHtLearners ? "Chargement…"
                      : habilHtLearners.length === 0 ? "Aucun apprenant inscrit"
                        : "Choisir un apprenant…"
                } />
              </SelectTrigger>
              <SelectContent>
                {habilHtLearners.map((l) => l.learner ? (
                  <SelectItem key={l.learner_id} value={l.learner_id}>
                    {l.learner.last_name} {l.learner.first_name}
                  </SelectItem>
                ) : null)}
          ...
          <Button onClick={handleGenerateHabilHt}
            disabled={!habilHtSessionId || !habilHtLearnerId || generatingHabilHt}
            className="w-full gap-2" size="lg">
            {generatingHabilHt ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><Zap className="h-4 w-4" />Générer avis habilitation BT-HT</>}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card className="border-purple-200 bg-purple-50/30">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Génère <strong>1 avis BT-HT par apprenant</strong>. Sortie : 1 ZIP.
          </p>
          <div>
            <Label htmlFor="habil-ht-batch-session-select" className="text-sm">Session</Label>
            <Select value={habilHtBatchSessionId} onValueChange={setHabilHtBatchSessionId}
              disabled={loadingSessions || generatingHabilHtBatch}>
              <SelectTrigger id="habil-ht-batch-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <Button onClick={handleGenerateHabilHtBatch}
            disabled={!habilHtBatchSessionId || generatingHabilHtBatch}
            className="w-full gap-2 bg-purple-600 hover:bg-purple-700" size="lg">
            {generatingHabilHtBatch ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><Package className="h-4 w-4" />Générer ZIP — tous les avis BT-HT</>}
          </Button>
        </CardContent>
      </Card>

      {lastHabilHtBatchResult && (
        <Card className={lastHabilHtBatchResult.failureCount === 0
          ? "border-green-200 bg-green-50/30" : "border-amber-200 bg-amber-50/30"}>
          <CardHeader>
            <CardTitle className={"text-base " + (lastHabilHtBatchResult.failureCount === 0 ? "text-green-900" : "text-amber-900")}>
              {lastHabilHtBatchResult.failureCount === 0 ? "✅" : "⚠️"} Dernier batch avis habilitation BT-HT
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div><strong>{lastHabilHtBatchResult.successCount}</strong> / {lastHabilHtBatchResult.totalLearners} avis générés</div>
            <div><strong>Latence totale :</strong> {lastHabilHtBatchResult.totalLatencyMs} ms</div>
            {lastHabilHtBatchResult.errors.length > 0 && (
              <div className="mt-3 space-y-1">
                <div className="flex items-center gap-1 text-amber-900 font-medium">
                  <AlertCircle className="h-4 w-4" /> Erreurs ({lastHabilHtBatchResult.errors.length})
                </div>
                <ul className="text-xs space-y-0.5 ml-5 list-disc">
                  {lastHabilHtBatchResult.errors.map((e) => (
                    <li key={e.learnerId}><strong>{e.learnerName}</strong> : {e.error}</li>
                  ))}
                </ul>
              </div>
      ...
      {lastHabilHtResult && (
        <Card className="border-green-200 bg-green-50/30">
          <CardHeader><CardTitle className="text-base text-green-900">✅ Dernier avis habilitation BT-HT</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div>
              <strong>Moteur :</strong> {lastHabilHtResult.engineUsed}
              {lastHabilHtResult.cacheHit && (
                <span className="ml-2 text-xs bg-amber-100 text-amber-800 px-2 py-0.5 rounded">⚡ Cache hit</span>
              )}
            </div>
            <div><strong>Latence :</strong> {lastHabilHtResult.latencyMs} ms</div>
            <div><strong>Taille PDF :</strong> {(lastHabilHtResult.fileSizeBytes / 1024).toFixed(1)} KB</div>
          </CardContent>
        </Card>
      )}

      {/* ────────── Avis Habilitation Électrique H0 B0 (Recyclage Non élec) ────────── */}

      <div className="pt-8">
        <h2 className="text-xl font-semibold flex items-center gap-2 mb-1">
          <Zap className="h-5 w-5 text-zinc-700" />
          Avis Habilitation Électrique — H0 B0 (Recyclage)
        </h2>
        <p className="text-sm text-muted-foreground">
          8ème variante — la plus minimaliste. Norme A2. Seulement{" "}
          <strong>BO-H0V</strong> sur les 2 lignes Non électrique, toutes les
          autres lignes vides. Typique formation Recyclage personnel non élec.
        </p>
      </div>

      <Card className="border-zinc-300 bg-zinc-50/40">
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <FlaskConical className="h-4 w-4 text-zinc-800" />
            Mode rapide — Données factices
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Patrick ATTLAN + HABILITATION ÉLECTRIQUE H0 B0 - RECYCLAGE.
          </p>
          <Button
            onClick={handleGenerateHabilH0B0Mock}
            disabled={generatingHabilH0B0 || generatingHabilH0B0Batch}
            className="w-full gap-2 bg-zinc-700 hover:bg-zinc-800"
          >
            {generatingHabilH0B0 ? <Loader2 className="h-4 w-4 animate-spin" /> : <FlaskConical className="h-4 w-4" />}
            Générer avis habilitation H0 B0 de test
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Avis habilitation H0 B0 — Données réelles (1 apprenant)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="habil-h0b0-session-select" className="text-sm">Session</Label>
            <Select value={habilH0B0SessionId} onValueChange={setHabilH0B0SessionId}
              disabled={loadingSessions || generatingHabilH0B0}>
              <SelectTrigger id="habil-h0b0-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <div>
            <Label htmlFor="habil-h0b0-learner-select" className="text-sm">Apprenant inscrit</Label>
            <Select value={habilH0B0LearnerId} onValueChange={setHabilH0B0LearnerId}
              disabled={!habilH0B0SessionId || loadingHabilH0B0Learners || generatingHabilH0B0}>
              <SelectTrigger id="habil-h0b0-learner-select" className="mt-1">
                <SelectValue placeholder={
                  !habilH0B0SessionId ? "Choisis d'abord une session"
                    : loadingHabilH0B0Learners ? "Chargement…"
                      : habilH0B0Learners.length === 0 ? "Aucun apprenant inscrit"
                        : "Choisir un apprenant…"
                } />
              </SelectTrigger>
              <SelectContent>
                {habilH0B0Learners.map((l) => l.learner ? (
                  <SelectItem key={l.learner_id} value={l.learner_id}>
                    {l.learner.last_name} {l.learner.first_name}
                  </SelectItem>
                ) : null)}
          ...
          <Button onClick={handleGenerateHabilH0B0}
            disabled={!habilH0B0SessionId || !habilH0B0LearnerId || generatingHabilH0B0}
            className="w-full gap-2" size="lg">
            {generatingHabilH0B0 ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><Zap className="h-4 w-4" />Générer avis habilitation H0 B0</>}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card className="border-purple-200 bg-purple-50/30">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Génère <strong>1 avis H0 B0 par apprenant</strong>. Sortie : 1 ZIP.
          </p>
          <div>
            <Label htmlFor="habil-h0b0-batch-session-select" className="text-sm">Session</Label>
            <Select value={habilH0B0BatchSessionId} onValueChange={setHabilH0B0BatchSessionId}
              disabled={loadingSessions || generatingHabilH0B0Batch}>
              <SelectTrigger id="habil-h0b0-batch-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <Button onClick={handleGenerateHabilH0B0Batch}
            disabled={!habilH0B0BatchSessionId || generatingHabilH0B0Batch}
            className="w-full gap-2 bg-purple-600 hover:bg-purple-700" size="lg">
            {generatingHabilH0B0Batch ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><Package className="h-4 w-4" />Générer ZIP — tous les avis H0 B0</>}
          </Button>
        </CardContent>
      </Card>

      {lastHabilH0B0BatchResult && (
        <Card className={lastHabilH0B0BatchResult.failureCount === 0
          ? "border-green-200 bg-green-50/30" : "border-amber-200 bg-amber-50/30"}>
          <CardHeader>
            <CardTitle className={"text-base " + (lastHabilH0B0BatchResult.failureCount === 0 ? "text-green-900" : "text-amber-900")}>
              {lastHabilH0B0BatchResult.failureCount === 0 ? "✅" : "⚠️"} Dernier batch avis habilitation H0 B0
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div><strong>{lastHabilH0B0BatchResult.successCount}</strong> / {lastHabilH0B0BatchResult.totalLearners} avis générés</div>
            <div><strong>Latence totale :</strong> {lastHabilH0B0BatchResult.totalLatencyMs} ms</div>
            {lastHabilH0B0BatchResult.errors.length > 0 && (
              <div className="mt-3 space-y-1">
                <div className="flex items-center gap-1 text-amber-900 font-medium">
                  <AlertCircle className="h-4 w-4" /> Erreurs ({lastHabilH0B0BatchResult.errors.length})
                </div>
                <ul className="text-xs space-y-0.5 ml-5 list-disc">
                  {lastHabilH0B0BatchResult.errors.map((e) => (
                    <li key={e.learnerId}><strong>{e.learnerName}</strong> : {e.error}</li>
                  ))}
                </ul>
              </div>
      ...
      {lastHabilH0B0Result && (
        <Card className="border-green-200 bg-green-50/30">
          <CardHeader><CardTitle className="text-base text-green-900">✅ Dernier avis habilitation H0 B0</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div>
              <strong>Moteur :</strong> {lastHabilH0B0Result.engineUsed}
              {lastHabilH0B0Result.cacheHit && (
                <span className="ml-2 text-xs bg-amber-100 text-amber-800 px-2 py-0.5 rounded">⚡ Cache hit</span>
              )}
            </div>
            <div><strong>Latence :</strong> {lastHabilH0B0Result.latencyMs} ms</div>
            <div><strong>Taille PDF :</strong> {(lastHabilH0B0Result.fileSizeBytes / 1024).toFixed(1)} KB</div>
          </CardContent>
        </Card>
      )}

      {/* ────────── Avis Habilitation Électrique H0 B0 INITIAL ────────── */}

      <div className="pt-8">
        <h2 className="text-xl font-semibold flex items-center gap-2 mb-1">
          <Zap className="h-5 w-5 text-stone-700" />
          Avis Habilitation Électrique — H0 B0 (Initial)
        </h2>
        <p className="text-sm text-muted-foreground">
          9ème variante — identique à H0 B0 Recyclage (#83) côté tableau
          (BO-H0V Non élec uniquement), mais avec la mention{" "}
          <strong>&ldquo;Initiale&rdquo; active</strong> et{" "}
          <s>&ldquo;Recyclage&rdquo;</s> barré en page 1.
        </p>
      </div>

      <Card className="border-stone-300 bg-stone-50/40">
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <FlaskConical className="h-4 w-4 text-stone-800" />
            Mode rapide — Données factices
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Patrick ATTLAN + HABILITATION ÉLECTRIQUE H0B0 - INITIAL.
          </p>
          <Button
            onClick={handleGenerateHabilH0B0InitMock}
            disabled={generatingHabilH0B0Init || generatingHabilH0B0InitBatch}
            className="w-full gap-2 bg-stone-700 hover:bg-stone-800"
          >
            {generatingHabilH0B0Init ? <Loader2 className="h-4 w-4 animate-spin" /> : <FlaskConical className="h-4 w-4" />}
            Générer avis habilitation H0 B0 Initial de test
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Avis habilitation H0 B0 Initial — Données réelles (1 apprenant)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="habil-h0b0init-session-select" className="text-sm">Session</Label>
            <Select value={habilH0B0InitSessionId} onValueChange={setHabilH0B0InitSessionId}
              disabled={loadingSessions || generatingHabilH0B0Init}>
              <SelectTrigger id="habil-h0b0init-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <div>
            <Label htmlFor="habil-h0b0init-learner-select" className="text-sm">Apprenant inscrit</Label>
            <Select value={habilH0B0InitLearnerId} onValueChange={setHabilH0B0InitLearnerId}
              disabled={!habilH0B0InitSessionId || loadingHabilH0B0InitLearners || generatingHabilH0B0Init}>
              <SelectTrigger id="habil-h0b0init-learner-select" className="mt-1">
                <SelectValue placeholder={
                  !habilH0B0InitSessionId ? "Choisis d'abord une session"
                    : loadingHabilH0B0InitLearners ? "Chargement…"
                      : habilH0B0InitLearners.length === 0 ? "Aucun apprenant inscrit"
                        : "Choisir un apprenant…"
                } />
              </SelectTrigger>
              <SelectContent>
                {habilH0B0InitLearners.map((l) => l.learner ? (
                  <SelectItem key={l.learner_id} value={l.learner_id}>
                    {l.learner.last_name} {l.learner.first_name}
                  </SelectItem>
                ) : null)}
          ...
          <Button onClick={handleGenerateHabilH0B0Init}
            disabled={!habilH0B0InitSessionId || !habilH0B0InitLearnerId || generatingHabilH0B0Init}
            className="w-full gap-2" size="lg">
            {generatingHabilH0B0Init ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><Zap className="h-4 w-4" />Générer avis habilitation H0 B0 Initial</>}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card className="border-purple-200 bg-purple-50/30">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Génère <strong>1 avis H0 B0 Initial par apprenant</strong>. Sortie : 1 ZIP.
          </p>
          <div>
            <Label htmlFor="habil-h0b0init-batch-session-select" className="text-sm">Session</Label>
            <Select value={habilH0B0InitBatchSessionId} onValueChange={setHabilH0B0InitBatchSessionId}
              disabled={loadingSessions || generatingHabilH0B0InitBatch}>
              <SelectTrigger id="habil-h0b0init-batch-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <Button onClick={handleGenerateHabilH0B0InitBatch}
            disabled={!habilH0B0InitBatchSessionId || generatingHabilH0B0InitBatch}
            className="w-full gap-2 bg-purple-600 hover:bg-purple-700" size="lg">
            {generatingHabilH0B0InitBatch ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><Package className="h-4 w-4" />Générer ZIP — tous les avis H0 B0 Initial</>}
          </Button>
        </CardContent>
      </Card>

      {lastHabilH0B0InitBatchResult && (
        <Card className={lastHabilH0B0InitBatchResult.failureCount === 0
          ? "border-green-200 bg-green-50/30" : "border-amber-200 bg-amber-50/30"}>
          <CardHeader>
            <CardTitle className={"text-base " + (lastHabilH0B0InitBatchResult.failureCount === 0 ? "text-green-900" : "text-amber-900")}>
              {lastHabilH0B0InitBatchResult.failureCount === 0 ? "✅" : "⚠️"} Dernier batch avis habilitation H0 B0 Initial
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div><strong>{lastHabilH0B0InitBatchResult.successCount}</strong> / {lastHabilH0B0InitBatchResult.totalLearners} avis générés</div>
            <div><strong>Latence totale :</strong> {lastHabilH0B0InitBatchResult.totalLatencyMs} ms</div>
            {lastHabilH0B0InitBatchResult.errors.length > 0 && (
              <div className="mt-3 space-y-1">
                <div className="flex items-center gap-1 text-amber-900 font-medium">
                  <AlertCircle className="h-4 w-4" /> Erreurs ({lastHabilH0B0InitBatchResult.errors.length})
                </div>
                <ul className="text-xs space-y-0.5 ml-5 list-disc">
                  {lastHabilH0B0InitBatchResult.errors.map((e) => (
                    <li key={e.learnerId}><strong>{e.learnerName}</strong> : {e.error}</li>
                  ))}
                </ul>
              </div>
      ...
      {lastHabilH0B0InitResult && (
        <Card className="border-green-200 bg-green-50/30">
          <CardHeader><CardTitle className="text-base text-green-900">✅ Dernier avis habilitation H0 B0 Initial</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div>
              <strong>Moteur :</strong> {lastHabilH0B0InitResult.engineUsed}
              {lastHabilH0B0InitResult.cacheHit && (
                <span className="ml-2 text-xs bg-amber-100 text-amber-800 px-2 py-0.5 rounded">⚡ Cache hit</span>
              )}
            </div>
      ...
      {/* ────────── Lettre de décharge de responsabilité (per session+learner) ────────── */}

      <div className="pt-8">
        <h2 className="text-xl font-semibold flex items-center gap-2 mb-1">
          <LogOut className="h-5 w-5 text-pink-700" />
          Lettre de décharge de responsabilité
        </h2>
        <p className="text-sm text-muted-foreground">
          Formalise un départ anticipé du stagiaire en cours de formation,
          déchargeant l&apos;organisme de toute responsabilité à partir de
          l&apos;heure annoncée. Champs heure / motif / lieu / date / signature
          à compléter à la main après impression.
        </p>
      </div>
      ...
      <Card className="border-pink-300 bg-pink-50/40">
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <FlaskConical className="h-4 w-4 text-pink-800" />
            Mode rapide — Données factices
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Patrick ATTLAN + POE Managers Proximité.
          </p>
          <Button
            onClick={handleGenerateDechargeMock}
            disabled={generatingDecharge || generatingDechargeBatch}
            className="w-full gap-2 bg-pink-700 hover:bg-pink-800"
          >
            {generatingDecharge ? <Loader2 className="h-4 w-4 animate-spin" /> : <FlaskConical className="h-4 w-4" />}
            Générer lettre décharge de test
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Lettre décharge — Données réelles (1 apprenant)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="decharge-session-select" className="text-sm">Session</Label>
            <Select value={dechargeSessionId} onValueChange={setDechargeSessionId}
              disabled={loadingSessions || generatingDecharge}>
              <SelectTrigger id="decharge-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <div>
            <Label htmlFor="decharge-learner-select" className="text-sm">Apprenant inscrit</Label>
            <Select value={dechargeLearnerId} onValueChange={setDechargeLearnerId}
              disabled={!dechargeSessionId || loadingDechargeLearners || generatingDecharge}>
              <SelectTrigger id="decharge-learner-select" className="mt-1">
                <SelectValue placeholder={
                  !dechargeSessionId ? "Choisis d'abord une session"
                    : loadingDechargeLearners ? "Chargement…"
                      : dechargeLearners.length === 0 ? "Aucun apprenant inscrit"
                        : "Choisir un apprenant…"
                } />
              </SelectTrigger>
              <SelectContent>
                {dechargeLearners.map((l) => l.learner ? (
                  <SelectItem key={l.learner_id} value={l.learner_id}>
                    {l.learner.last_name} {l.learner.first_name}
                  </SelectItem>
                ) : null)}
          ...
          <Button onClick={handleGenerateDecharge}
            disabled={!dechargeSessionId || !dechargeLearnerId || generatingDecharge}
            className="w-full gap-2" size="lg">
            {generatingDecharge ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><LogOut className="h-4 w-4" />Générer lettre décharge</>}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card className="border-purple-200 bg-purple-50/30">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Génère <strong>1 lettre décharge par apprenant</strong>. Sortie : 1 ZIP.
          </p>
          <div>
            <Label htmlFor="decharge-batch-session-select" className="text-sm">Session</Label>
            <Select value={dechargeBatchSessionId} onValueChange={setDechargeBatchSessionId}
              disabled={loadingSessions || generatingDechargeBatch}>
              <SelectTrigger id="decharge-batch-session-select" className="mt-1">
                <SelectValue placeholder={loadingSessions ? "Chargement…" : "Choisir une session…"} />
              </SelectTrigger>
              <SelectContent>
                {sessions.map((s) => (
          ...
          <Button onClick={handleGenerateDechargeBatch}
            disabled={!dechargeBatchSessionId || generatingDechargeBatch}
            className="w-full gap-2 bg-purple-600 hover:bg-purple-700" size="lg">
            {generatingDechargeBatch ? <><Loader2 className="h-4 w-4 animate-spin" />Génération en cours…</>
              : <><Package className="h-4 w-4" />Générer ZIP — toutes les lettres</>}
          </Button>
        </CardContent>
      </Card>

      {lastDechargeBatchResult && (
        <Card className={lastDechargeBatchResult.failureCount === 0
          ? "border-green-200 bg-green-50/30" : "border-amber-200 bg-amber-50/30"}>
          <CardHeader>
            <CardTitle className={"text-base " + (lastDechargeBatchResult.failureCount === 0 ? "text-green-900" : "text-amber-900")}>
              {lastDechargeBatchResult.failureCount === 0 ? "✅" : "⚠️"} Dernier batch lettres décharge
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div><strong>{lastDechargeBatchResult.successCount}</strong> / {lastDechargeBatchResult.totalLearners} lettres générées</div>
            <div><strong>Latence totale :</strong> {lastDechargeBatchResult.totalLatencyMs} ms</div>
            {lastDechargeBatchResult.errors.length > 0 && (
              <div className="mt-3 space-y-1">
                <div className="flex items-center gap-1 text-amber-900 font-medium">
                  <AlertCircle className="h-4 w-4" /> Erreurs ({lastDechargeBatchResult.errors.length})
                </div>
                <ul className="text-xs space-y-0.5 ml-5 list-disc">
                  {lastDechargeBatchResult.errors.map((e) => (
                    <li key={e.learnerId}><strong>{e.learnerName}</strong> : {e.error}</li>
                  ))}
                </ul>
              </div>
      ...
      {lastDechargeResult && (
        <Card className="border-green-200 bg-green-50/30">
          <CardHeader><CardTitle className="text-base text-green-900">✅ Dernière lettre décharge</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div>
              <strong>Moteur :</strong> {lastDechargeResult.engineUsed}
              {lastDechargeResult.cacheHit && (
                <span className="ml-2 text-xs bg-amber-100 text-amber-800 px-2 py-0.5 rounded">⚡ Cache hit</span>
              )}
            </div>
