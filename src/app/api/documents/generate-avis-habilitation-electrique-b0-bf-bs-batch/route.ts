/**
 * POST /api/documents/generate-avis-habilitation-electrique-b0-bf-bs-batch
 *
 * Variante B0-BF-BS (norme NF C 18-510 A2). 1 avis par apprenant → ZIP
  ...
  try {
    ...
    const tasks = learners.map(async (learner) => {
      ...
      const result = await service.generate({
        ...
        docType: "avis_habilitation_electrique_b0_bf_bs",
        html: resolvedHtml,
        cacheInputs: {
          doc_type: "avis_habilitation_electrique_b0_bf_bs",
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`avis-habilitation-b0-bf-bs-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating avis habilitation électrique B0-BF-BS batch") },
      { status: 500 },
    );
  }
}
