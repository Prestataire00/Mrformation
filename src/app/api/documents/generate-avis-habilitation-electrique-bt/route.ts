/**
 * POST /api/documents/generate-avis-habilitation-electrique-bt
 *
 * Variante INITIAL BT (norme NF C 18-510 A2). Body : `{ sessionId, learnerId }`.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating avis habilitation électrique BT") },
      { status: 500 },
    );
  }
}
