/**
 * POST /api/documents/generate-emargements-individuels-batch
 *
 * Génère N feuilles d'émargement individuelles (1 par apprenant) → ZIP
  ...
  try {
    ...
    const tasks = learners.map(async (learner) => {
      const context: ResolveContext = {
        ...
        // slot-aware dès qu'il y a des formation_time_slots (joints ci-dessus)
        // et lit signaturesBySlotPerson. Sans ce champ, aucune signature.
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`emargement-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    if (errors.length > 0) {
      ...
      zip.file(
        "_erreurs.txt",
        `Échec pour ${errors.length} apprenant(s) sur ${learners.length} :\n\n${report}\n`,
      );
    }
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating émargements individuels batch") },
      { status: 500 },
    );
  }
}
