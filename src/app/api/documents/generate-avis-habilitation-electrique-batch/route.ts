/**
 * POST /api/documents/generate-avis-habilitation-electrique-batch
 *
 * Génère 1 avis par apprenant → ZIP fail-soft. Body : `{ sessionId }`.
 */
  ...
  try {
    ...
    const tasks = learners.map(async (learner) => {
      ...
      const result = await service.generate({
        ...
        docType: "avis_habilitation_electrique",
        html: resolvedHtml,
        cacheInputs: {
          doc_type: "avis_habilitation_electrique",
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`avis-habilitation-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating avis habilitation électrique batch") },
      { status: 500 },
    );
  }
}
