/**
 * POST /api/documents/send-certificats-travail-hauteur-batch-email
 *
 * Thin-wrapper Story F2.x — délègue à batchSendDocsEmail.
 * Génère et envoie les certificats de travail en hauteur par email à tous les destinataires de la session.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "sending certificats travail hauteur batch email") },
      { status: 500 },
    );
  }
}
