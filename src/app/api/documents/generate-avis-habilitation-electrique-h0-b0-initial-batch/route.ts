/**
 * POST /api/documents/generate-avis-habilitation-electrique-h0-b0-initial-batch
 *
 * Variante H0 B0 Initial (norme A2 — Non électrique uniquement, "Initiale"
 * actif). 1 avis par apprenant → ZIP fail-soft. Body : `{ sessionId }`.
 */
  ...
  try {
    ...
    const tasks = learners.map(async (learner) => {
      ...
      const result = await service.generate({
        ...
        docType: "avis_habilitation_electrique_h0_b0_initial",
        html: resolvedHtml,
        cacheInputs: {
          doc_type: "avis_habilitation_electrique_h0_b0_initial",
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`avis-habilitation-h0-b0-initial-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating avis habilitation électrique H0-B0 Initial batch") },
      { status: 500 },
    );
  }
}
