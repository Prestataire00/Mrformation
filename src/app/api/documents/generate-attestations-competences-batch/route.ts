/**
 * POST /api/documents/generate-attestations-competences-batch
 *
 * Génère 1 attestation par apprenant → ZIP fail-soft.
 * Body : `{ sessionId }`.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: session } = await supabase
      .from("sessions")
      .select("*, training:trainings(*), program:programs(*), formation_trainers:formation_trainers(trainer:trainers(*))")
    ...
    const tasks = learners.map(async (learner) => {
      ...
      const result = await service.generate({
        ...
        docType: "attestation_competences",
        html: resolvedHtml,
        cacheInputs: {
          doc_type: "attestation_competences",
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`attestation-competences-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating attestations compétences batch") },
      { status: 500 },
    );
  }
}
