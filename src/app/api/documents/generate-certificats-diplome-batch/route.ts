/**
 * POST /api/documents/generate-certificats-diplome-batch
 *
 * Génère N certificats diplôme (1 par apprenant inscrit) → ZIP fail-soft.
 * Body : `{ sessionId }`.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const tasks = learners.map(async (learner) => {
      const certificateCode = generateCertificateCode(learner.id, body.sessionId!);
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating certificats diplôme batch") },
      { status: 500 },
    );
  }
}
