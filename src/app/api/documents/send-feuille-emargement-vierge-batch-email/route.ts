/**
 * POST /api/documents/send-feuille-emargement-vierge-batch-email
 *
 * Thin-wrapper Story F1.x — délègue à batchSendDocsEmail.
 * Génère et envoie la feuille d'émargement vierge par email à tous les destinataires de la session.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "sending feuille emargement vierge batch email") },
      { status: 500 },
    );
  }
}
