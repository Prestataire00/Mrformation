/**
 * POST /api/documents/generate-resultats-evaluations
 *
 * Génère un PDF "Résultats des évaluations" pour 1 apprenant.
 * Body : `{ sessionId, learnerId }`. Score calculé depuis
 * `questionnaire_responses` + `questions.options.correct_answer` via le
 * helper `loadEvaluationResults`.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [entity, evaluationResults] = await Promise.all([
      loadEntitySettings(supabase, profile.entity_id),
      loadEvaluationResults(supabase, body.sessionId, body.learnerId),
    ]);
    ...
    const result = await service.generate({
      ...
      html: resolvedHtml,
      cacheInputs: {
        doc_type: "resultats_evaluations",
        session_id: body.sessionId,
        learner_id: body.learnerId,
        session_updated_at: (session as { updated_at?: string }).updated_at ?? null,
        custom_variables: {
          evals_count: String(evaluationResults.length),
          evals_completed: String(evaluationResults.filter((r) => r.status !== "non_complete").length),
        },
      },
      options: {
        format: "A4",
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating résultats évaluations") },
      { status: 500 },
    );
  }
}
