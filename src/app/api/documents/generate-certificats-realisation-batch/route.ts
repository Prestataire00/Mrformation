/**
 * POST /api/documents/generate-certificats-realisation-batch
 *
 * Génère N certificats de réalisation (1 par apprenant inscrit à la session)
 * et empaquette dans un ZIP. Fail-soft. Body : `{ sessionId: UUID }`.
 *
 * Chaque certificat porte le nom de l'entreprise présentatrice (via
 * enrollment.client_id), donc les apprenants de différentes entreprises
 * d'une session INTER auront chacun le nom de leur entreprise propre.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Apprenants + client_id de chaque enrollment + clients en parallèle
    const { data: enrollments, error: enrErr } = await supabase
      .from("enrollments")
      .select("client_id, learner:learners(*)")
    ...
    // Charge tous les clients distincts en 1 query
    ...
    const tasks = validEnrolled.map(async (enr) => {
    ...
    settled.forEach((outcome, idx) => {
      const learner = validEnrolled[idx].learner!;
      const name = `${learner.last_name} ${learner.first_name}`;

      if (outcome.status === "fulfilled") {
        zip.file(`certificat-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    if (errors.length > 0) {
      ...
      zip.file(
        "_erreurs.txt",
        `Échec de génération pour ${errors.length} apprenant(s) sur ${validEnrolled.length} :\n\n${report}\n`,
      );
    }
    ...
    return NextResponse.json({
      ...
      totalLearners: validEnrolled.length,
      successCount,
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating certificats batch") },
      { status: 500 },
    );
  }
}
