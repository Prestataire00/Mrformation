/**
 * POST /api/documents/generate-attestation-competences
 *
 * Génère 1 attestation de compétences pour 1 apprenant.
 * Body : `{ sessionId, learnerId }`.
 *
 * Charge session avec training (pour objectifs) + formation_trainers
 * (pour signature) + enrollment (pour learner).
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating attestation compétences") },
      { status: 500 },
    );
  }
}
