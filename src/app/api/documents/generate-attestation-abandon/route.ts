/**
 * POST /api/documents/generate-attestation-abandon
 *
 * Body : `{ sessionId, learnerId }`. Réutilise signedLearnerIds pour
 * compter les heures effectivement suivies avant l'abandon.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Audit 24/07 : heures réellement suivies avant l'abandon — calculées par
    // créneau émargé quand c'est possible (sinon repli legacy présence
    // intégrale). Particulièrement pertinent ici : un abandon en cours de
    // formation a par définition une présence partielle.
    ...
    const result = await service.generate({
      ...
      docType: "attestation_abandon_formation",
      html: resolvedHtml,
      cacheInputs: {
        doc_type: "attestation_abandon_formation",
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating attestation abandon") },
      { status: 500 },
    );
  }
}
