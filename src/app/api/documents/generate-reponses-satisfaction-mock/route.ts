/**
 * POST /api/documents/generate-reponses-satisfaction-mock
 *
 * Mock : session Managers Proximité avec satisfaction agrégée + KPIs
 * Qualiopi + résultats évaluations factices.
 */
...
export async function POST(_request: NextRequest) {
  try {
    ...
    const result = await service.generate({
      ...
      docType: "reponses_satisfaction_mock",
      html: resolvedHtml,
      cacheInputs: {
        doc_type: "reponses_satisfaction_mock",
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating mock réponses satisfaction") },
      { status: 500 },
    );
  }
}
