/**
 * POST /api/documents/generate-avis-habilitation-electrique-bt-batch
 *
 * Variante INITIAL BT (norme NF C 18-510 A2). 1 avis par apprenant → ZIP
  ...
  try {
    ...
    const tasks = learners.map(async (learner) => {
      ...
      const result = await service.generate({
        ...
        docType: "avis_habilitation_electrique_bt",
        html: resolvedHtml,
        cacheInputs: {
          doc_type: "avis_habilitation_electrique_bt",
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`avis-habilitation-bt-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating avis habilitation électrique BT batch") },
      { status: 500 },
    );
  }
}
