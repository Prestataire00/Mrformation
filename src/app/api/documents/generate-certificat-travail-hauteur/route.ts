/**
 * POST /api/documents/generate-certificat-travail-hauteur
 *
 * Body : `{ sessionId, learnerId }`. Charge enrollment + learner +
 * client (entreprise présentatrice).
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating certificat travail hauteur") },
      { status: 500 },
    );
  }
}
