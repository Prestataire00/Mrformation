/**
 * POST /api/documents/generate-reponses-evaluations
 *
 * Génère 1 PDF "Réponses aux évaluations" pour 1 session — variante
 * simplifiée (2 tableaux seulement : satisfaction + évaluations).
 * Body : `{ sessionId }`.
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await service.generate({
      ...
      docType: "reponses_evaluations",
      html: resolvedHtml,
      cacheInputs: {
        doc_type: "reponses_evaluations",
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating réponses évaluations") },
      { status: 500 },
    );
  }
}
