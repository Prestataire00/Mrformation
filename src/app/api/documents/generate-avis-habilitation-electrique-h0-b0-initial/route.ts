/**
 * POST /api/documents/generate-avis-habilitation-electrique-h0-b0-initial
 *
 * Variante H0 B0 Initial (norme NF C 18-510 A2 — Non électrique uniquement,
 * "Initiale" actif). Body : `{ sessionId, learnerId }`.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating avis habilitation électrique H0-B0 Initial") },
      { status: 500 },
    );
  }
}
