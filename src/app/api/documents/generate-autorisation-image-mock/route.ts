/**
 * POST /api/documents/generate-autorisation-image-mock
 *
 * Mock : Patrick ATTLAN, formation Managers Proximité.
 */
...
export async function POST(_request: NextRequest) {
  try {
    ...
    const result = await service.generate({
      ...
      docType: "autorisation_image_mock",
      html: resolvedHtml,
      cacheInputs: {
        doc_type: "autorisation_image_mock",
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating mock autorisation image") },
      { status: 500 },
    );
  }
}
