/**
 * POST /api/documents/send-resultats-evaluations-batch-email
 *
 * Thin-wrapper Story F2.x — délègue à batchSendDocsEmail.
 * Génère et envoie les résultats des évaluations par email à tous les destinataires de la session.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "sending resultats evaluations batch email") },
      { status: 500 },
    );
  }
}
