/**
 * POST /api/documents/generate-avis-habilitation-electrique-bt-ht-batch
 *
 * Variante BT-HT (HAUTE TENSION, norme A2). 1 avis par apprenant → ZIP
  ...
  try {
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`avis-habilitation-bt-ht-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating avis habilitation électrique BT-HT batch") },
      { status: 500 },
    );
  }
}
