/**
 * POST /api/documents/generate-attestation-assiduite-mock
 *
 * Génère une attestation d'assiduité avec DONNÉES FACTICES (Patrick ATTLAN,
 * 14h réalisées sur 14h, taux 100%).
 */
...
export async function POST(_request: NextRequest) {
  try {
    ...
    const context: ResolveContext = {
      ...
      learner: mockLearner,
      entity,
      // signedLearnerIds non fourni → fallback "présent" → 14h, taux 100%
    };
    ...
    const result = await service.generate({
      ...
      docType: "attestation_assiduite_mock",
      html: resolvedHtml,
      cacheInputs: {
        doc_type: "attestation_assiduite_mock",
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating mock attestation assiduité") },
      { status: 500 },
    );
  }
}
