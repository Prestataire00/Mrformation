/**
 * POST /api/documents/generate-decharges-responsabilite-batch
 *
 * Génère 1 décharge par apprenant → ZIP fail-soft.
 * Body : `{ sessionId }`.
 */
...
} from "@/lib/templates/decharge-responsabilite";
import {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: enrollments, error: enrErr } = await supabase
      .from("enrollments").select("learner:learners(*)")
    ...
    const tasks = learners.map(async (learner) => {
      ...
      const resolvedHtml = resolveDocumentVariables(DECHARGE_RESPONSABILITE_HTML, context);
      const resolvedFooter = resolveDocumentVariables(DECHARGE_RESPONSABILITE_FOOTER_TEMPLATE, context);

      const result = await service.generate({
        entityId: profile.entity_id,
        docType: "decharge_responsabilite",
        html: resolvedHtml,
        cacheInputs: {
          doc_type: "decharge_responsabilite",
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`decharge-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating décharges responsabilité batch") },
      { status: 500 },
    );
  }
}
