/**
 * POST /api/documents/send-attestations-abandon-batch-email
 *
 * Thin-wrapper Story F2.x — délègue à batchSendDocsEmail.
 * Génère et envoie les attestations d'abandon de formation par email à tous les destinataires de la session.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "sending attestations abandon batch email") },
      { status: 500 },
    );
  }
}
