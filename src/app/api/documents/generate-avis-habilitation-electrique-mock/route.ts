/**
 * POST /api/documents/generate-avis-habilitation-electrique-mock
 *
 * Mock : Patrick ATTLAN + formation Habilitation B1V-B2V-BR-BC.
 */
...
} from "@/lib/templates/avis-habilitation-electrique";
import {
...
export async function POST(_request: NextRequest) {
  try {
    ...
    const resolvedHtml = resolveDocumentVariables(AVIS_HABILITATION_ELECTRIQUE_HTML, context);
    const resolvedFooter = resolveDocumentVariables(AVIS_HABILITATION_ELECTRIQUE_FOOTER_TEMPLATE, context);
    ...
    const result = await service.generate({
      ...
      docType: "avis_habilitation_electrique_mock",
      html: resolvedHtml,
      cacheInputs: {
        doc_type: "avis_habilitation_electrique_mock",
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating mock avis habilitation électrique") },
      { status: 500 },
    );
  }
}
