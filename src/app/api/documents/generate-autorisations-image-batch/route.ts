/**
 * POST /api/documents/generate-autorisations-image-batch
 *
 * Génère 1 autorisation droit à l'image par apprenant → ZIP fail-soft.
 * Body : `{ sessionId }`.
 */
...
} from "@/lib/templates/autorisation-image";
import {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const tasks = learners.map(async (learner) => {
      ...
      const resolvedHtml = resolveDocumentVariables(AUTORISATION_IMAGE_HTML, context);
      const resolvedFooter = resolveDocumentVariables(AUTORISATION_IMAGE_FOOTER_TEMPLATE, context);

      const result = await service.generate({
        entityId: profile.entity_id,
        docType: "autorisation_image",
        html: resolvedHtml,
        cacheInputs: {
          doc_type: "autorisation_image",
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`autorisation-image-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating autorisations image batch") },
      { status: 500 },
    );
  }
}
