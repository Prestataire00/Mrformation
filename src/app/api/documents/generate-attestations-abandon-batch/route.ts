/**
 * POST /api/documents/generate-attestations-abandon-batch
 *
 * Génère 1 attestation d'abandon par apprenant → ZIP fail-soft.
 * Body : `{ sessionId }`.
 */
  ...
  try {
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`attestation-abandon-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating attestations abandon batch") },
      { status: 500 },
    );
  }
}
