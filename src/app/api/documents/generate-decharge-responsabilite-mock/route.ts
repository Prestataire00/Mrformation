/**
 * POST /api/documents/generate-decharge-responsabilite-mock
 *
 * Mock : Patrick ATTLAN quitte la formation Managers Proximité.
 */
...
export async function POST(_request: NextRequest) {
  try {
    ...
    const result = await service.generate({
      ...
      docType: "decharge_responsabilite_mock",
      html: resolvedHtml,
      cacheInputs: {
        doc_type: "decharge_responsabilite_mock",
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating mock décharge responsabilité") },
      { status: 500 },
    );
  }
}
