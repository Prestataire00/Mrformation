/**
 * POST /api/documents/send-planning-semaine-batch-email
 *
 * Thin-wrapper Story F1.x — délègue à batchSendDocsEmail.
 * Génère et envoie le planning de la semaine par email à tous les destinataires de la session.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "sending planning semaine batch email") },
      { status: 500 },
    );
  }
}
