/**
 * POST /api/documents/generate-contrats-engagement-batch
 *
 * Génère 1 contrat d'engagement par apprenant → ZIP fail-soft.
 * Body : `{ sessionId }`.
 */
  ...
  try {
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`contrat-engagement-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating contrats engagement batch") },
      { status: 500 },
    );
  }
}
