/**
 * POST /api/documents/generate-attestation-aipr-mock
 *
 * Mock : Patrick ATTLAN + UNICIL + ticket "MARSEILLE" + date 10/01/2025.
 */
...
export async function POST(_request: NextRequest) {
  try {
    ...
    const result = await service.generate({
      ...
      docType: "attestation_aipr_mock",
      html: resolvedHtml,
      cacheInputs: {
        doc_type: "attestation_aipr_mock",
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating mock AIPR") },
      { status: 500 },
    );
  }
}
