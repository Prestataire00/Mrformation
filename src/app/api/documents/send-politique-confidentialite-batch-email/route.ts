/**
 * POST /api/documents/send-politique-confidentialite-batch-email
 *
 * Thin-wrapper Story F1.x — délègue à batchSendDocsEmail.
 * Génère et envoie la politique de confidentialité (RGPD) par email à tous les destinataires de la session.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "sending politique confidentialite batch email") },
      { status: 500 },
    );
  }
}
