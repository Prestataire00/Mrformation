/**
 * POST /api/documents/generate-attestations-assiduite-batch
 *
 * Génère N attestations d'assiduité (1 par apprenant) → ZIP fail-soft.
 * Body : `{ sessionId }`. Calcul présent/absent depuis table signatures.
 */
  ...
  try {
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`attestation-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating attestations assiduité batch") },
      { status: 500 },
    );
  }
}
