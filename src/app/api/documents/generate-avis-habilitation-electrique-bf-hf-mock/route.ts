/**
 * POST /api/documents/generate-avis-habilitation-electrique-bf-hf-mock
 *
 * Mock variante BF-HF / H0-B0 (Travaux d'Ordre NON ELECTRIQUE) :
 * Patrick ATTLAN + Habilitation BF-HF / H0-B0.
 */
...
export async function POST(_request: NextRequest) {
  try {
    ...
    const mockSession: Session = {
      id: "mock-session-habil-bf-hf",
      entity_id: profile.entity_id,
      title: "Habilitation electrique - BF/HF",
      ...
      planned_hours: 7,
      formation_trainers: [
        { trainer: { id: "mock-trainer-bf-hf-1", first_name: "Brigitte", last_name: "MARTINEAU", email: null, entity_id: profile.entity_id } },
      ],
    ...
    const result = await service.generate({
      ...
      docType: "avis_habilitation_electrique_bf_hf_mock",
      html: resolvedHtml,
      cacheInputs: {
        doc_type: "avis_habilitation_electrique_bf_hf_mock",
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating mock avis habilitation BF-HF") },
      { status: 500 },
    );
  }
}
