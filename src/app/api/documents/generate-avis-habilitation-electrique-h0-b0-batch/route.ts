/**
 * POST /api/documents/generate-avis-habilitation-electrique-h0-b0-batch
 *
 * Variante H0 B0 Recyclage (norme A2 — Non électrique uniquement). 1 avis
 * par apprenant → ZIP fail-soft. Body : `{ sessionId }`.
 */
  ...
  try {
    ...
    const tasks = learners.map(async (learner) => {
      ...
      const result = await service.generate({
        ...
        docType: "avis_habilitation_electrique_h0_b0",
        html: resolvedHtml,
        cacheInputs: {
          doc_type: "avis_habilitation_electrique_h0_b0",
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`avis-habilitation-h0-b0-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating avis habilitation électrique H0-B0 batch") },
      { status: 500 },
    );
  }
}
