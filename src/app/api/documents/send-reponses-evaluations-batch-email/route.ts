/**
 * POST /api/documents/send-reponses-evaluations-batch-email
 *
 * Thin-wrapper Story F2.x — délègue à batchSendDocsEmail.
 * Génère et envoie les réponses aux évaluations par email à tous les destinataires de la session.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "sending reponses evaluations batch email") },
      { status: 500 },
    );
  }
}
