/**
 * POST /api/documents/generate-attestations-aipr-batch
 *
 * Génère 1 attestation AIPR par apprenant → ZIP fail-soft.
 * Body : `{ sessionId }`.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Charge tous les clients en 1 query
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating attestations AIPR batch") },
      { status: 500 },
    );
  }
}
