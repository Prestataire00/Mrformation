/**
 * POST /api/documents/generate-lettres-decharge-batch
 *
 * Génère 1 lettre de décharge par apprenant → ZIP fail-soft.
 * Body : `{ sessionId }`.
 */
  ...
  try {
    ...
    const tasks = learners.map(async (learner) => {
      ...
      const result = await service.generate({
        ...
        docType: "lettre_decharge_responsabilite",
        html: resolvedHtml,
        cacheInputs: {
          doc_type: "lettre_decharge_responsabilite",
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`lettre-decharge-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating lettres décharge batch") },
      { status: 500 },
    );
  }
}
