/**
 * POST /api/documents/generate-avis-habilitation-electrique-b1v-b2v-br-batch
 *
 * Variante B1V-B2V-BR (sans BC, norme A1). 1 avis par apprenant → ZIP
 * fail-soft. Body : `{ sessionId }`.
 */
  ...
  try {
    ...
    const tasks = learners.map(async (learner) => {
      ...
      const result = await service.generate({
        ...
        docType: "avis_habilitation_electrique_b1v_b2v_br",
        html: resolvedHtml,
        cacheInputs: {
          doc_type: "avis_habilitation_electrique_b1v_b2v_br",
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`avis-habilitation-b1v-b2v-br-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating avis habilitation électrique B1V-B2V-BR batch") },
      { status: 500 },
    );
  }
}
