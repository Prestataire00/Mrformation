/**
 * POST /api/documents/send-attestations-aipr-batch-email
 *
 * Thin-wrapper Story F2.x — délègue à batchSendDocsEmail.
 * Génère et envoie les attestations AIPR par email à tous les destinataires de la session.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "sending attestations aipr batch email") },
      { status: 500 },
    );
  }
}
