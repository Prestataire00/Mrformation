/**
 * POST /api/documents/generate-resultats-evaluations-batch
 *
 * Génère 1 PDF résultats évaluations par apprenant inscrit → ZIP fail-soft.
 * Body : `{ sessionId }`.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const tasks = learners.map(async (learner) => {
      const evaluationResults = await loadEvaluationResults(supabase, body.sessionId!, learner.id);
    ...
    settled.forEach((outcome, idx) => {
      ...
      if (outcome.status === "fulfilled") {
        zip.file(`resultats-evaluations-${slugify(name)}.pdf`, outcome.value.result.buffer);
        successCount += 1;
      } else {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "generating résultats évaluations batch") },
      { status: 500 },
    );
  }
}
