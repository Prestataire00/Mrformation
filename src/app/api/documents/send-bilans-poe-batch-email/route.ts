/**
 * POST /api/documents/send-bilans-poe-batch-email
 *
 * Thin-wrapper Story F1.x — délègue à batchSendDocsEmail.
 * Génère et envoie les bilans POE par email à tous les destinataires de la session.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: sanitizeError(err, "sending bilans poe batch email") },
      { status: 500 },
    );
  }
}
