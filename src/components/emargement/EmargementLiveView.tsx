...
/**
 * Mode présentation émargement (QR + statuts live), piloté par `sessionId`.
 * Partagé entre l'espace admin (`/admin/formations/[id]/emargement-live`) et
 * l'espace formateur (`/trainer/sessions/[id]/emargement-live`). Les routes API
 * `/api/emargement/{live-status,slots}` autorisent déjà le rôle trainer.
 */
export default function EmargementLiveView({
  sessionId,
  onQuit,
}: {
  sessionId: string;
  ...
  if (error && slots.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-gray-50 p-4">
        ...
        <Button onClick={onQuit}>Retour</Button>
      </div>
    );
  }
  ...
  if (needsTokens) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 bg-gray-50 p-8 text-center">
        ...
        <Button variant="outline" onClick={onQuit}>← Retour</Button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      ...
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        <Button variant="ghost" size="sm" onClick={onQuit} className="text-white hover:bg-white/10 gap-1">
