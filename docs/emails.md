...
- ~~**em-c-10** : génération PDF serveur facture + devis~~ — **✅ résolu 2026-05-28**. Implémentation Puppeteer + templates HTML (`src/lib/templates/facture-email.ts` + `devis-email.ts`) appelés depuis `resolveFacture`/`resolveDevis` dans `email-attachments-resolver.ts`. Pipeline devis migré vers `enqueueEmail` dans `crm/quotes/process-reminders/route.ts` (cohérence avec invoices). Events Netlify : `email_attachment_facture_generated` / `email_attachment_devis_generated` (succès, avec `latency_ms` + `size_bytes`), `email_attachment_*_generation_failed` (level critical, retourne null sans bloquer l'envoi).
