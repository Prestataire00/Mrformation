...
- 2026-07-22 : Revue fresh-context — **APPROVED**, zéro finding bloquant/majeur. Reviewer a vérifié la séquentialité AD-14 (snapshot `batchDialogInvoices` stable → pas de re-loop ; flag `stale` complet), la cohérence algébrique `getBatchIneligibilityReason` null ⇔ `isBatchSelectable`, la pureté/totalité de `summarizeBatchPreviews`, la garde null, le reset entité, AD-22, le CTA désactivé et le périmètre (aucune route/select serveur). 3 observations mineures/informatives sans action (TVA mixte théoriquement impossible en mono-entité ; skeleton hérité de 3.1 ; garde tsc). Status → done.
