...
## BPF-2.3 câblage rapport — findings review 3 agents (2026-07-02)

- **D1 — RÉFUTÉ puis reclassé (investigation 2026-07-02, cf. `investigations/avoirs-cadre-c-bpf-investigation.md`).** La prémisse « avoirs à date nulle → exclus → CA surévalué » est **fausse** : la route POST invoices résout `invoice_date || today` de façon inconditionnelle (`route.ts:145-149`) → les avoirs sont datés (date d'émission), inclus dans le Cadre C, **total CA correct**, et `avoirs_orphelins` fonctionne. Données prod : 4 avoirs, 0 sans date. **Vrais défauts (réduits) :** (A) `funding_type` NULL sur les 4 avoirs → ils se soustraient de la ligne 11 « Autres » au lieu de la ligne d'origine (répartition Cerfa faussée, total juste) — **déjà remédiables via le DataGapsPanel** livré ; (B) date d'émission vs facture d'origine → **décision métier tranchée : date d'émission + explication UX** (aucun changement de date). Note : les 4 avoirs sont **sans `parent_invoice_id`** (importés/legacy ou surcharge RPC — à trancher). **Fix béton** = prévention à la création (persister le parent + hériter funding_type) + UX d'explication + catégorisation des 4 existants via le panneau. Pas de backfill de dates.
- **D2 (MOYENNE) : Cadre C annuel (`fiscalYear`) vs cadres F sur plage `dateFrom/dateTo`.** `fetchBPFData` ignore la plage custom et prend l'exercice entier ; `sessionQuery`/`enrollQuery` filtrent par `start_date` dans [dateFrom, dateTo]. Divergent seulement si l'utilisateur rétrécit sous l'année (BPF = déclaration annuelle, défaut = année pleine → cohérent). Fix propre : passer `dateFrom/dateTo` à `fetchBPFData`, ou piloter tout le rapport par `fiscalYear`. À défaut, note UI quand la plage ≠ année pleine.
- **D3 (BASSE) : `avoirs_orphelins` / `signatures_legacy` calculés mais hors badge `totalGaps` du panneau.** DataGapsPanel (pré-existant BPF-2.2) ne les compte pas dans « N éléments » ni dans un onglet ; `signatures_legacy` est « info seulement » par la story FR-2.1.9. Un rapport peut sembler propre en app tout en portant ces trous (visibles seulement dans le résumé PDF). Ajouter un onglet lecture-seule ou documenter l'exclusion.
- **D4 (BASSE) : KPI « CA formation » vs `totalProduits()` vs overrides manuels peuvent diverger.** La KPI lit `computedSectionC` (au fetch), le total affiché lit l'état `sectionC`, et les overrides (`overrides.section_c`) n'affectent ni l'un ni l'autre (pattern pré-existant). Après un override de cellule Cadre C, jusqu'à 3 chiffres peuvent différer. Dériver KPI + split + total d'une même source, ou documenter que le split reflète les factures brutes.
- **D5 (BASSE, défense en profondeur) : mutations pré-existantes du DataGapsPanel sans filtre `entity_id`.** `updateInvoiceBPF`, `updateEnrollmentBPF`, `batchUpdateEnrollmentsBPF`, `updateTrainingBPF`, `updateFormationTrainerCost` updatent par `.eq/.in("id", …)` sans `.eq("entity_id", …)`. IDs issus d'un fetch entity-scoped (pas exploitable via l'UI), RLS `entity_isolation` sur formation_invoices en backstop — mais mémoire projet : RLS prod parfois `allow_all`. Le nouveau `batchConfirmInvoiceDates` a été durci (garde `entity_id`). Durcir les autres : thread `entityId` (invoices/trainings ont la colonne ; enrollments/formation_trainers scopés via session).

## Onglet BPF par-formation — différés review 3 agents (2026-07-03)

- **OB1 (MOYENNE) : vue mono-année (factures à cheval sur 2 exercices).** L'onglet BPF (`TabBpf.tsx`) calcule l'année depuis `session.start_date` et filtre les factures sur cette seule année ; une session déc. N → janv. N+1 avec factures dans les 2 années ne montre que celles de N (pas de sélecteur d'année). Cohérent avec le modèle annuel du rapport global, mais peut afficher « prêt à déclarer » alors qu'une facture N+1 de la session est un trou dans le BPF N+1 (invisible dans l'onglet). Fix : sélecteur d'année (Should) ou avertir si la session a des factures hors de l'année affichée.
- **OB2 (BASSE, défense en profondeur) : garde de validation côté serveur.** La règle du vert (valider seulement si `totalGaps=0`) est appliquée côté UI (bouton désactivé) ; `validateSessionBPF` écrit `bpf_validated_at` sans re-vérifier. `bpf_validated_at` est donc *advisory* (pas une preuve de complétude). Si ce flag devient load-bearing (export officiel), re-dériver les trous côté service avant d'écrire.
- **OB3 (cadrage, pas un bug) : totaux stagiaires non additifs entre formations.** Un apprenant sur 3 formations compte 1 fois au global mais 1 fois dans chaque onglet → la somme des onglets ≠ total global. Inhérent au scope par-formation (« contribution de cette formation »). Éventuellement une note UI.
