...
- **D1 (HAUTE / correctness) : avoirs à `invoice_date` nul exclus du Cadre C → CA surévalué.** `TabFinances.tsx:469` crée un avoir avec `invoice_date: null` (et `amount = -Math.abs(...)`), mais la migration `bpf_qualification_fields.sql:10` pose `invoice_date NOT NULL`. `fetchBPFData` filtre `.gte/.lte("invoice_date")` → toute ligne à date nulle est écartée par Postgres → les avoirs ne se soustraient jamais du Cadre C, et `avoirs_orphelins` (détection cross-année) est de fait mort. **Pré-existant** (convention TabFinances + service BPF-2.1 + migration), révélé par le câblage invoices. Fix dédié : migration pour dater les avoirs depuis leur facture parent (ou `created_at`) + corriger TabFinances pour ne plus écrire `null`, OU fetch avoirs séparément sans filtre de date. Vérifier d'abord l'état réel prod (colonne nullable ou non — sinon la création d'avoir échoue).
