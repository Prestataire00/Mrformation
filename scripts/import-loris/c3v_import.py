#!/usr/bin/env python3
"""
Import Loris XLSX → Supabase (C3V FORMATION uniquement)
...
REPORT_PATH = Path(__file__).parent / "last_c3v_import_report.json"

FILES = {
    "clients": "Listing entreprise.xlsx",
    "learners": "Listing apprenants.xlsx",
    "trainings_sessions": "Suivi_de_l_activite_.xlsx",
    "formation_trainers": "Suivi_de_l_activite__des_formateurs.xlsx",
    "enrollments": "Suivi_de_l_activite__des_stagiaires.xlsx",
    "crm_quotes": "__C3V_AUCUN_DEVIS__.xlsx",
    "formation_invoices": "suivi_des_factures.xlsx",
}
        ...
        if not data:
            ...
            if legacy_charge_invoices is None:
                ...
                for r in existing_charges:
                    key = (C3V_ENTITY_ID, r.get("session_id"),
                ...
                for base, cp in charge_candidates:
                    key = (C3V_ENTITY_ID, cp["session_id"], base, canon_charge_amount(cp["amount"]))
